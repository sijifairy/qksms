 ...
 *
 * <ul>
 *   <li>The Proxy-Relay (Through Push messages)
 *   <li>The composer/viewer activities of the MMS Client (Through intents)
 * </ul>
 *
 * The TransactionService runs locally in the same process as the application. It contains a
 * HandlerThread to which messages are posted from the intent-receivers of this application.
 *
 * <p><b>IMPORTANT</b>: This is currently the only instance in the system in which simultaneous
 * connectivity to both the mobile data network and a Wi-Fi network is allowed. This makes the code
 * for handling network connectivity somewhat different than it is in other applications. In
 * particular, we want to be able to send or receive MMS messages when a Wi-Fi connection is active
 * (which implies that there is no connection to the mobile data network). This has two main
 * consequences:
 *
 * <ul>
 *   <li>Testing for current network connectivity ({@link android.net.NetworkInfo#isConnected()} is
 *       not sufficient. Instead, the correct test is for network availability ({@link
 *       android.net.NetworkInfo#isAvailable()}).
 *   <li>If the mobile data network is not in the connected state, but it is available, we must
 *       initiate setup of the mobile data connection, and defer handling the MMS transaction until
...
public class TransactionService extends Service implements Observer {
  ...
  /**
   * Used to identify notification intents broadcasted by the TransactionService when a Transaction
   * is completed.
   */
  ...
  /**
   * Action for the Intent which is sent when the user turns on the auto-retrieve setting. This
   * service gets started to auto-retrieve any undownloaded messages.
   */
  public static final String ACTION_ENABLE_AUTO_RETRIEVE =
      "android.intent.action.ACTION_ENABLE_AUTO_RETRIEVE";

  /**
   * Used as extra key in notification intents broadcasted by the TransactionService when a
   * Transaction is completed (TRANSACTION_COMPLETED_ACTION intents). Allowed values for this key
   * are: TransactionState.INITIALIZED, TransactionState.SUCCESS, TransactionState.FAILED.
   */
  ...
  public int onStartCommand(final Intent intent, int flags, int startId) {
    if (intent != null) {
      ...
      //                        Cursor locationQuery = getContentResolver().query(contentUri, new
      // String[]{Telephony.Mms.CONTENT_LOCATION, Telephony.Mms._ID}, null, null, "date desc");
      //
      //                        if (locationQuery != null && locationQuery.moveToFirst()) {
      //                            Timber.v("grabbing content location url");
      //                            downloadLocation =
      // locationQuery.getString(locationQuery.getColumnIndex(Telephony.Mms.CONTENT_LOCATION));
      //                        }
      //
      //                        Timber.v("creating request with url: " + downloadLocation);
      //                        DownloadRequest request = new DownloadRequest(downloadLocation,
      // contentUri, null, null, null);
      //                        MmsNetworkManager manager = new
      // MmsNetworkManager(TransactionService.this);
  ...
  public void onNewIntent(Intent intent, int serviceId) {
    ...
    Timber.v(
        "onNewIntent: serviceId: " + serviceId + ": " + intent.getExtras() + " intent=" + intent);
    ...
    if (ACTION_ONALARM.equals(action)
      ...
      Cursor cursor =
          PduPersister.getPduPersister(this).getPendingMessages(System.currentTimeMillis());
      if (cursor != null) {
        try {
          ...
          while (cursor.moveToNext()) {
            ...
            try {
              Uri uri =
                  ContentUris.withAppendedId(Mms.CONTENT_URI, cursor.getLong(columnIndexOfMsgId));
              com.android.mms.transaction.DownloadManager.getInstance()
                  .downloadMultimediaMessage(
                      this, PushReceiver.getContentLocation(this, uri), uri, false);
        ...
        if (mPending.size() > 0) {
          ...
          Message msg =
              mServiceHandler.obtainMessage(
                  EVENT_HANDLE_NEXT_PENDING_TRANSACTION, transaction.getConnectionSettings());
          mServiceHandler.sendMessage(msg);
  ...
  private final class ServiceHandler extends Handler {
      ...
      switch (msg.what) {
        ...
        case EVENT_CONTINUE_MMS_CONNECTIVITY:
          ...
          try {
            ...
            if (result != 0) {
              Timber.v(
                  "Extending MMS connectivity returned "
                      + result
        ...
        case EVENT_TRANSACTION_REQUEST:
          ...
          try {
            ...
            Timber.v(
                "EVENT_TRANSACTION_REQUEST MmscUrl="
                    + args.getMmscUrl()
                    + " proxy port: "
                    + args.getProxyAddress());
            ...
            if (mmsc != null) {
              transactionSettings =
                  new TransactionSettings(mmsc, args.getProxyAddress(), args.getProxyPort());
            } else {
            ...
            Timber.v(
                "handle EVENT_TRANSACTION_REQUEST: transactionType="
                    + transactionType
                    + " "
                    + decodeTransactionType(transactionType));
                ...
                if (uri != null) {
                  transaction =
                      new NotificationTransaction(
                          TransactionService.this, serviceId, transactionSettings, uri);
                } else {
              ...
              case Transaction.RETRIEVE_TRANSACTION:
                transaction =
                    new RetrieveTransaction(
                        TransactionService.this, serviceId, transactionSettings, args.getUri());
                ...
                com.android.mms.transaction.DownloadManager.getInstance()
                    .downloadMultimediaMessage(
                        ...
                        ((RetrieveTransaction) transaction)
                            .getContentLocation(TransactionService.this, u),
                        u,
                        false);
                return;
            }
     ...
     * @return {@code true} if process has begun or will begin. {@code false} if the transaction
     *     should be discarded.
     * @throws java.io.IOException if connectivity for MMS traffic could not be established.
     */
  ...
  private void renewMmsConnectivity() {
    ...
    mServiceHandler.sendMessageDelayed(
        mServiceHandler.obtainMessage(EVENT_CONTINUE_MMS_CONNECTIVITY), APN_EXTENSION_WAIT);
  }
      ...
      if (mmsNetworkInfo == null) {
        ...
        if (mmsNetworkInfo.isConnected()) {
          TransactionSettings settings =
              new TransactionSettings(TransactionService.this, mmsNetworkInfo.getExtraInfo());
