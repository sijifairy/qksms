...
public class FiveStarRateDialog extends DefaultButtonDialog2 implements View.OnClickListener {
    ...
    public static final String DESKTOP_PREFS = BaseApplication.getContext().getPackageName() + "_desktop"; // Main process
    ...
    public static final String PREF_KEY_FIVE_STAR_SHOWED_AFTER_SEND_MSG = "PREF_KEY_FIVE_STAR_SHOWED_AFTER_SEND_MSG";
    public static final String PREF_KEY_FIVE_STAR_SHOWED_AFTER_SEND_EMOJI = "PREF_KEY_FIVE_STAR_SHOWED_AFTER_SEND_EMOJI";
    ...
    private void loadAnimation() {
        ...
        try {
            mAnimationLoadTask = LottieComposition.Factory.fromAssetFileName(getContext(),
                    "lottie/five_star_rating.json", lottieComposition -> {
                        if (!mAnimViewShowed) {
    ...
    protected void onClickPositiveButton(View v) {
        if (mCurrentPosition >= 0) {
            SmsAnalytics.logEvent("Alert_FiveStar_Submit_BtnClicked", "type", (mCurrentPosition + 1) + "star");
            if (mCurrentPosition >= MAX_POSITION) {
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=com.messenger.sms.infinity.theme"))
                        .addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY | Intent.FLAG_ACTIVITY_NEW_DOCUMENT | Intent.FLAG_ACTIVITY_MULTIPLE_TASK);

                try {
                    mActivity.startActivity(intent);
                } catch (ActivityNotFoundException e) {
                    mActivity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("http://play.google.com/store/apps/details?id=com.messenger.sms.infinity.theme")));
                }
    ...
    private void logStarClick(int position) {
        SmsAnalytics.logEvent("Alert_FiveStar_Star_Clicked", "type", (position + 1) + "star",
