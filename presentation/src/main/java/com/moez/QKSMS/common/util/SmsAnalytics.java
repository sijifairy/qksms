...
/**
 * For v1.4.5 (78) release: to cut down number of events reported to Flurry, we log most events only
 * to Fabric. Give {@code true} for {@code alsoLogToFlurry} to log an event also to Flurry.
 */
public class SmsAnalytics {
  ...
  public static void logEvent(
      final String eventID, boolean alsoLogToFlurry, Map<String, String> eventValues) {
    try {
  ...
  private static void onLogEvent(
      String eventID, boolean alsoLogToFlurry, Map<String, String> eventValues) {
    String eventDescription = getEventInfoDescription(eventID, alsoLogToFlurry, eventValues);
    Log.d(TAG, eventDescription);
  }

  private static String getEventInfoDescription(
      String eventID, boolean alsoLogToFlurry, Map<String, String> eventValues) {
    ...
    if (values.length() > 0) {
      values =
          new StringBuilder(
              ": "
                  + values.substring(
                      0, values.length() - 2)); // At ": " at front and remove ", " in the end
    }
  ...
  private static void assertEventIdOrKeyOrValueLengthNoMoreThan40(String eventIdOrKeyOrValue) {
    ...
    if (eventIdOrKeyOrValue.length() > 40) {
      throw new AssertionError(
          "The length of " + eventIdOrKeyOrValue + " already more than 40 which is illegal!!!!!");
    }
  }
  ...
  private static void assertEventIdOrKeyStartLegal(String eventIdOrKey) {
    ...
    if (eventIdOrKey.startsWith("firebase_")
      ...
      throw new AssertionError(
          eventIdOrKey + " is start with \"firebase_\" or \"google_\" or \"ga_\" which is illegal");
    }
  }
  ...
  private static void assertEventKeyNoMoreThan25(String eventId, String key) {
    ...
    if (!keyList.contains(key)) {
      ...
      if (keyList.size() > 25) {
        throw new AssertionError(
            "The parameters of event must not exceed 25，"
                + "but the parameters of "
                + eventId
                + " has been more than 25!!! Please check it.");
      }
