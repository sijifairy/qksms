...
public class SmartspaceCard {
  ...
  static SmartspaceCard cD(Context context, i iVar, boolean z) {
    if (iVar != null) {
      try {
        Intent parseUri =
            TextUtils.isEmpty(iVar.de.cG.cZ) ? null : Intent.parseUri(iVar.de.cG.cZ, 0);
        ...
        if (bitmap != null) {
          bitmap =
              ShadowGenerator.getInstance(context)
                  .recreateIcon(
                      bitmap,
                      false,
                      new BlurMaskFilter(
                          (float)
                              Utilities.pxFromDp(3.0f, context.getResources().getDisplayMetrics()),
        ...
        return new SmartspaceCard(
            context, iVar.de, parseUri, z, bitmap, iVar.dc, iVar.df, iVar.dh, iVar.dg);
      } catch (Throwable e) {
      ...
      case 2:
        {
          try {
            ...
            forward.setComponent(
                new ComponentName(forward.getPackage(), "amirz.aidlbridge.GoogleDestForwarder"));
