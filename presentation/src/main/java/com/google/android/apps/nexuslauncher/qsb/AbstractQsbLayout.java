...
public abstract class AbstractQsbLayout extends FrameLayout
    implements LauncherLayoutChangeListener, OnClickListener, OnSharedPreferenceChangeListener {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int calculateCellWidth =
        DeviceProfile.calculateCellWidth(bw, deviceProfile.inv.numHotseatIcons);
    int round = Math.round(((float) deviceProfile.iconSizePx) * 0.92f);
    setMeasuredDimension(
        ((bw - (calculateCellWidth - round)) + getPaddingLeft()) + getPaddingRight(),
  ...
  public void draw(Canvas canvas) {
    if (mShadowBitmap == null) {
      ...
      mShadowBitmap =
          createBitmap(((float) iconBitmapSize) / 96f, ((float) iconBitmapSize) / 48f, mColor);
    }
  ...
  protected void loadDimensions(Bitmap bitmap, Canvas canvas) {
    ...
    drawWithDimensions(
        bitmap, canvas, i2 - 5, i2 + 5, paddingLeft + ((float) i2), width2 - ((float) i2));
  }

  private void drawWithDimensions(
      Bitmap bitmap, Canvas canvas, int srcLeft, int srcRight, float destLeft, float destRight) {
  ...
  public void onClick(View view) {
    if (view == mMicIconView) {
      fallbackSearch(
          Utilities.getDevicePrefs(getContext()).getBoolean("opa_enabled", true)
  ...
  private void loadPreferences(SharedPreferences sharedPreferences) {
    ...
    mMicIconView.setVisibility(
        opa && getContext().getPackageManager().queryIntentActivities(opaIntent, 0).isEmpty()
            ? View.GONE
            : View.VISIBLE);

    ((ImageView) mMicIconView)
        .setImageResource(opa ? R.drawable.ic_poodle_color : R.drawable.ic_mic_color);
    requestLayout();
  }
}
