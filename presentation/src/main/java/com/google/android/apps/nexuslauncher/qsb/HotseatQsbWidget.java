...
public class HotseatQsbWidget extends AbstractQsbLayout {
  ...
  static int getBottomMargin(Launcher launcher) {
    ...
    return res.getDimensionPixelSize(R.dimen.qsb_hotseat_bottom_margin)
        + (insets.bottom == 0
  ...
  private void setColors() {
    View.inflate(
        new ContextThemeWrapper(
            getContext(),
            mIsDefaultLiveWallpaper ? R.style.HotseatQsbTheme_Colored : R.style.HotseatQsbTheme),
        R.layout.qsb_hotseat_content,
        this);
    bz(
        getResources()
            .getColor(
                mIsDefaultLiveWallpaper
  ...
  private void doOnClick() {
    ...
    if (mActivity.getGoogleNow().startSearch(f.build(), f.getExtras())) {
      ...
      devicePrefs
          .edit()
          .putInt(
              "key_hotseat_qsb_tap_count", devicePrefs.getInt("key_hotseat_qsb_tap_count", 0) + 1)
          .apply();
      playQsbAnimation();
    } else {
      getContext()
  ...
  protected void noGoogleAppSearch() {
    final Intent searchIntent =
        new Intent("com.google.android.apps.searchlite.WIDGET_ACTION")
            .setComponent(
        ...
        try {
          ...
          LauncherAppsCompat.getInstance(getContext())
              .showAppDetailsForProfile(
                  new ComponentName(GOOGLE_QSB, ".SearchActivity"), Process.myUserHandle());
  ...
  private void playAnimation(boolean hideWorkspace, boolean longDuration) {
    ...
    if (hideWorkspace) {
      ...
      ObjectAnimator ofFloat =
          ObjectAnimator.ofFloat(dragLayer, View.TRANSLATION_Y, translationValues);
