...
public class PixelBridge {
  public static boolean isInstalled(Context context) {
    ...
    try {
      PackageInfo info =
          manager.getPackageInfo(LauncherClient.BRIDGE_PACKAGE, PackageManager.GET_SIGNATURES);
  ...
  /**
   * Enforce signature check to prevent malicious edits or recompilations from being used as a
   * bridge.
   *
   * @param signatures Extracted signatures from the bridge package.
   * @return True if all signatures match the config, false if at least one does not match or the
