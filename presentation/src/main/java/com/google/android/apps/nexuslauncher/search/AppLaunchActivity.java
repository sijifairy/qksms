  ...
  private void startUri(Uri uri) {
    try {
      ...
      if (!getPackageManager().isSafeMode() || Utilities.isSystemApp(this, dVar.getIntent())) {
        if (dl.user.equals(android.os.Process.myUserHandle())) {
          ...
          LauncherAppsCompat.getInstance(this)
              .startActivityForProfile(
                  dl.componentName, dl.user, getIntent().getSourceBounds(), null);
        }
        View view = new View(this);
        view.setTag(dVar);
        int i = 2;
        LauncherModel.Callbacks callback =
            LauncherAppState.getInstance(this).getModel().getCallback();
        ...
        new LogContainerProvider(
                this, TextUtils.isEmpty(predictionRank) ? -1 : Integer.parseInt(predictionRank))
            .addView(view);
        return;
      }
