...
public class FlingToDeleteHelper {
  ...
  public FlingToDeleteHelper(Launcher launcher) {
    ...
    mFlingToDeleteThresholdVelocity =
        launcher.getResources().getDimensionPixelSize(R.dimen.drag_flingToDeleteMinVelocity);
  }
  ...
  /**
   * Same as {@link #recordMotionEvent}. It creates a temporary {@link MotionEvent} object using
   * {@param event} for tracking velocity.
   */
  public void recordDragEvent(long dragStartTime, DragEvent event) {
    ...
    MotionEvent emulatedEvent =
        MotionEvent.obtain(
            dragStartTime, SystemClock.uptimeMillis(), motionAction, event.getX(), event.getY(), 0);
  ...
  private float getAngleBetweenVectors(PointF vec1, PointF vec2) {
    return (float)
        Math.acos(((vec1.x * vec2.x) + (vec1.y * vec2.y)) / (vec1.length() * vec2.length()));
  }
}
