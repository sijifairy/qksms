...
public class WorkspaceStateTransitionAnimation {
  ...
  public AnimatorSet getAnimationToState(
    ...
    animateWorkspace(states, animated, workspaceDuration, layerViews, accessibilityEnabled);
  ...
  private void animateWorkspace(
      final TransitionStates states,
    ...
    float finalHotseatAlpha =
        (states.stateIsNormal || states.stateIsSpringLoaded || states.stateIsNormalHidden)
            ? 1f
            : 0f;
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (animated) {
        ...
        if (initialAlpha != finalAlpha) {
          Animator alphaAnim =
              ObjectAnimator.ofFloat(cl.getShortcutsAndWidgets(), View.ALPHA, finalAlpha);
          alphaAnim.setDuration(duration).setInterpolator(mZoomInInterpolator);
          mStateAnimator.play(alphaAnim);
        }
        if (oldBackgroundAlpha != 0 || finalBackgroundAlpha != 0) {
          ValueAnimator bgAnim =
              ObjectAnimator.ofFloat(
                  cl, "backgroundAlpha", oldBackgroundAlpha, finalBackgroundAlpha);
    ...
    if (animated) {
      ...
      if (finalOverviewPanelAlpha != overviewPanel.getAlpha()) {
        Animator overviewPanelAlpha =
            ObjectAnimator.ofFloat(overviewPanel, View.ALPHA, finalOverviewPanelAlpha);
        overviewPanelAlpha.addListener(
            new AlphaUpdateListener(overviewPanel, accessibilityEnabled));
  ...
  private void animateBackgroundGradient(TransitionStates states, boolean animated, int duration) {
    ...
    float finalAlpha =
        states.stateIsNormal || states.stateIsNormalHidden ? 0 : mWorkspaceScrimAlpha;
    ...
    if (finalAlpha != startAlpha) {
      if (animated) {
        ...
        bgFadeOutAnimation.addUpdateListener(
