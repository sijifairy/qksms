 ...
 *
 * <p>The fraction is 1 / number of pages and the position is based on the progress of the page
 * scroll.
 */
public class PageIndicatorLineCaret extends PageIndicator {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    canvas.drawRect(
        lineLeft, canvas.getHeight() - mLineHeight, lineRight, canvas.getHeight(), mLinePaint);
  }
  ...
  /**
   * The line's color will be: - mostly opaque white if the hotseat is white (ignoring alpha) -
  ...
  public void updateColor(ExtractedColors extractedColors) {
    ...
    if (color != Color.TRANSPARENT) {
      ...
      if (color == Color.BLACK) {
        ...
        Log.e(
            TAG,
            "Setting workspace page indicators to an unsupported color: #"
  ...
  private void animateToNumPages(int numPages) {
    setupAndRunAnimation(
        ObjectAnimator.ofFloat(this, NUM_PAGES, numPages), NUM_PAGES_ANIMATOR_INDEX);
  }

  private void animateToTotalScroll(int totalScroll) {
    setupAndRunAnimation(
        ObjectAnimator.ofInt(this, TOTAL_SCROLL, totalScroll), TOTAL_SCROLL_ANIMATOR_INDEX);
  }

  /**
   * Starts the given animator and stores it in the provided index in {@link #mAnimators} until the
   * animation ends.
   *
   * <p>If an animator is already at the index (i.e. it is already playing), it is canceled and
  ...
  private void setupAndRunAnimation(ValueAnimator animator, final int animatorIndex) {
    ...
    mAnimators[animatorIndex].addListener(
