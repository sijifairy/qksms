...
/**
 * A {@link FrameLayout} that contains a header, main view and a footer. The main view contains the
 * icon and text (title + subtext) of the first notification. The footer contains: A list of just
 * the icons of all the notifications past the first one.
 *
...
public class NotificationItemView extends PopupItemView implements LogContainerProvider {
  ...
  /**
   * This method is used to calculate the height to remove when dismissing the last notification. We
   * subtract the height of the footer in this case since the footer should be gone or in the
  ...
  public int getHeightMinusFooter() {
    ...
    int excessFooterHeight =
        mFooter.getHeight()
            - getResources().getDimensionPixelSize(R.dimen.notification_empty_footer_height);
  ...
  public Animator animateHeightRemoval(int heightToRemove, boolean shouldRemoveFromTop) {
    ...
    anim.play(
        new RoundedRectRevealOutlineProvider(
                getBackgroundRadius(), getBackgroundRadius(), startRect, endRect, mRoundedCorners)
            .createRevealAnimator(this, false));

    View bottomGutter = findViewById(R.id.gutter_bottom);
    if (bottomGutter != null && bottomGutter.getVisibility() == VISIBLE) {
      Animator translateGutter =
          ObjectAnimator.ofFloat(bottomGutter, TRANSLATION_Y, -heightToRemove);
    ...
    if (palette != null) {
      if (mNotificationHeaderTextColor == Notification.COLOR_DEFAULT) {
        mNotificationHeaderTextColor =
            IconPalette.resolveContrastColor(
                getContext(),
  ...
  public void trimNotifications(final List<String> notificationKeys) {
    boolean dismissedMainNotification =
        !notificationKeys.contains(mMainView.getNotificationInfo().notificationKey);
    if (dismissedMainNotification && !mAnimatingNextIcon) {
      ...
      mFooter.animateFirstNotificationTo(
          sTempRect,
