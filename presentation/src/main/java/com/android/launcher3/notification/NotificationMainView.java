...
/**
 * A {@link android.widget.FrameLayout} that contains a single notification, e.g. icon + title +
 * text.
 */
public class NotificationMainView extends FrameLayout implements SwipeDetector.Listener {
  ...
  public void applyNotificationInfo(
      NotificationInfo mainNotification, View iconView, boolean animate) {
  ...
  public boolean onDrag(float displacement, float velocity) {
    setTranslationX(
        canChildBeDismissed() ? displacement : OverScroll.dampedScroll(displacement, getWidth()));
    animate().cancel();
    return true;
  }
  ...
  public void onDragEnd(float velocity, boolean fling) {
    ...
    long duration =
        SwipeDetector.calculateDuration(
            velocity, (endTranslation - getTranslationX()) / getWidth());
    animate()
