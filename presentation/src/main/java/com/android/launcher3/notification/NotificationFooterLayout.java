...
/**
 * A {@link FrameLayout} that contains only icons of notifications. If there are more than {@link
 * #MAX_FOOTER_NOTIFICATIONS} icons, we add a "..." overflow.
 */
public class NotificationFooterLayout extends FrameLayout {
  ...
  public NotificationFooterLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    int ellipsisSpace =
        res.getDimensionPixelSize(R.dimen.horizontal_ellipsis_offset)
  ...
  /**
   * Keep track of the NotificationInfo, and then update the UI when {@link
  ...
  /**
   * Adds icons and potentially overflow text for all of the NotificationInfo's added using {@link
  ...
  private void removeViewFromIconRow(View child) {
    ...
    if (mIconRow.getChildCount() == 0) {
      ...
      PopupContainerWithArrow popup =
          PopupContainerWithArrow.getOpen(Launcher.getLauncher(getContext()));
      if (popup != null) {
        final int newHeight =
            getResources().getDimensionPixelSize(R.dimen.notification_empty_footer_height);
        Animator collapseFooter =
            popup.reduceNotificationViewHeight(
                getHeight() - newHeight,
                getResources().getInteger(R.integer.config_removeNotificationViewDuration));
        collapseFooter.addListener(
            new AnimatorListenerAdapter() {
              @Override
              public void onAnimationEnd(Animator animation) {
