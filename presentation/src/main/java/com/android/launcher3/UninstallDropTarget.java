...
public class UninstallDropTarget extends ButtonDropTarget {
  ...
  private static ComponentName getUninstallTarget(Context context, ItemInfo item) {
    ...
    if (intent != null) {
      LauncherActivityInfo info =
          LauncherAppsCompat.getInstance(context).resolveActivity(intent, user);
  ...
  public void completeDrop(final DragObject d) {
    DropTargetResultCallback callback =
        d.dragSource instanceof DropTargetResultCallback
            ? (DropTargetResultCallback) d.dragSource
            : null;
  ...
  public static boolean startUninstallActivity(
    ...
    if (cn == null) {
      ...
      try {
        Intent i =
            Intent.parseUri(launcher.getString(R.string.delete_package_intent), 0)
   ...
   *
   * <p>Since there is no direct callback for an uninstall request, we check the package existence
            ...
            public void run() {
              ...
              boolean uninstallSuccessful =
                  LauncherAppsCompat.getInstance(launcher)
                          .getApplicationInfo(
                              cn.getPackageName(), PackageManager.MATCH_UNINSTALLED_PACKAGES, user)
                      == null;
