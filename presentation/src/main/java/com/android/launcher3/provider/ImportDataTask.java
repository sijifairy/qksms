...
public class ImportDataTask {
  ...
  private ImportDataTask(Context context, String sourceAuthority) {
    mContext = context;
    mOtherScreensUri =
        Uri.parse("content://" + sourceAuthority + "/" + WorkspaceScreens.TABLE_NAME);
    mOtherFavoritesUri = Uri.parse("content://" + sourceAuthority + "/" + Favorites.TABLE_NAME);
  }

  public boolean importWorkspace() throws Exception {
    ArrayList<Long> allScreens =
        LauncherDbUtils.getScreenIdsFromCursor(
            mContext
                .getContentResolver()
                .query(
                    mOtherScreensUri,
    ...
    for (int i = 0; i < count; i++) {
      ...
      screenOps.add(
          ContentProviderOperation.newInsert(LauncherSettings.WorkspaceScreens.CONTENT_URI)
              .withValues(v)
              .build());
    }
    ...
    LauncherSettings.Settings.call(
        mContext.getContentResolver(), LauncherSettings.Settings.METHOD_CLEAR_EMPTY_DB_FLAG);
    return true;
  }

  /**
   * 1) Imports all the workspace entries from the source provider. 2) For home screen entries, maps
   * the screen id based on {@param screenIdMap} 3) In the end fills any holes in hotseat with items
   * from default hotseat layout.
   */
  private void importWorkspaceItems(long firsetScreenId, LongSparseArray<Long> screenIdMap)
      throws Exception {
    String profileId =
        Long.toString(
            UserManagerCompat.getInstance(mContext).getSerialNumberForUser(Process.myUserHandle()));
    ...
    try (Cursor c =
      ...
      while (c.moveToNext()) {
          ...
          default:
            if (!mValidFolders.get(container)) {
              FileLog.d(
                  TAG,
                  String.format(
                      "Skipping item %d, type %d not in a valid folder %d", id, type, container));
              continue;
            }
        }
        ...
        insertOperations.add(
            ContentProviderOperation.newInsert(Favorites.CONTENT_URI).withValues(values).build());
        if (container < 0) {
    ...
    if (hotseatItems.size() < myHotseatCount) {
      ...
      HotseatParserCallback parserCallback =
          new HotseatParserCallback(
              hotseatTargetApps, hotseatItems, insertOperations, maxId + 1, myHotseatCount);
      new HotseatLayoutParser(mContext, parserCallback).loadLayout(null, new ArrayList<Long>());
  ...
  public static boolean performImportIfPossible(Context context) throws Exception {
    ...
    devicePrefs
        .edit()
        .remove(KEY_DATA_IMPORT_SRC_PKG)
        .remove(KEY_DATA_IMPORT_SRC_AUTHORITY)
        .commit();
    ...
    for (ProviderInfo info :
        context
            .getPackageManager()
            .queryContentProviders(null, context.getApplicationInfo().uid, 0)) {
      ...
      if (sourcePackage.equals(info.packageName)) {
        ...
        if (sourceAuthority.equals(info.authority)) {
          if (TextUtils.isEmpty(info.readPermission)
              || context.checkPermission(info.readPermission, Process.myPid(), Process.myUid())
  ...
  private static class HotseatParserCallback implements LayoutParserCallback {
    ...
    HotseatParserCallback(
        HashSet<String> existingApps,
        LongArrayMap<Object> existingItems,
        ArrayList<ContentProviderOperation> outOps,
        int startItemId,
        int requiredSize) {
