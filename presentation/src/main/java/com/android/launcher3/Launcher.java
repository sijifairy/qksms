...
public class Launcher extends BaseActivity
  ...
  /**
   * Holds extra information required to handle a result from an external call, like {@link
   * #startActivityForResult(Intent, int)} or {@link #requestPermissions(String[], int)}
   */
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    overrideTheme(
        ...
        wallpaperColorInfo.isTransparent());
  ...
  /**
   * To be overridden by subclasses to populate the custom content container and call {@link
   * #addToCustomContentPage}. This will only be invoked if {@link #hasCustomContentToLeft()} is
   * {@code true}.
   */
  ...
  /**
   * Invoked by subclasses to signal a change to the {@link #addToCustomContentPage} value to ensure
   * the custom content page is added or removed if necessary.
   */
  ...
  private void handleActivityResult(
    ...
    Runnable exitSpringLoaded =
        new Runnable() {
          @Override
          public void run() {
            exitSpringLoadedDragModeDelayed(
                (resultCode != RESULT_CANCELED), EXIT_SPRINGLOADED_MODE_SHORT_TIMEOUT, null);
          }
        };
    ...
    boolean isWidgetDrop =
        (requestCode == REQUEST_PICK_APPWIDGET || requestCode == REQUEST_CREATE_APPWIDGET);
    ...
    if (isWidgetDrop) {
      ...
      if (appWidgetId < 0 || resultCode == RESULT_CANCELED) {
        Log.e(
            TAG,
            "Error: appWidgetId (EXTRA_APPWIDGET_ID) was not "
  ...
  public void onRequestPermissionsResult(
    ...
    if (requestCode == REQUEST_PERMISSION_CALL_PHONE
      ...
      if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
        ...
        Toast.makeText(
                this,
                getString(R.string.msg_no_phone_permission, getString(R.string.derived_app_name)),
                Toast.LENGTH_SHORT)
            .show();
      }
    }
  ...
  void completeTwoStageWidgetDrop(
    ...
    if (resultCode == RESULT_OK) {
      ...
      final AppWidgetHostView layout =
          mAppWidgetHost.createView(
              this, appWidgetId, requestArgs.getWidgetHandler().getProviderInfo(this));
      boundWidget = layout;
      onCompleteRunnable =
          new Runnable() {
            @Override
    ...
    if (mShouldFadeInScrim && mDragLayer.getBackground() != null) {
      ...
      mScrimAnimator =
          ObjectAnimator.ofInt(
              mDragLayer.getBackground(), LauncherAnimUtils.DRAWABLE_ALPHA, 0, 255);
  ...
  protected void onResume() {
    ...
    if (mBindOnResumeCallbacks.size() > 0) {
      ...
      if (DEBUG_RESUME_TIME) {
        Log.d(
            TAG,
            "Time spent processing callbacks in onResume: "
  ...
  public interface LauncherOverlay {
    ...
    /**
     * Scroll progress, between 0 and 100, when the user scrolls beyond the leftmost screen (or in
     * the case of RTL, the rightmost screen).
     */
  ...
  public void addToCustomContentPage(
      View customContent, CustomContentCallbacks callbacks, String description) {
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    ...
    if (!handled && acceptFilter() && isKeyNotWhitespace) {
      boolean gotKey =
          TextKeyListener.getInstance().onKeyDown(mWorkspace, mDefaultKeySsb, keyCode, event);
  ...
  private void restoreState(Bundle savedState) {
    ...
    State state =
        (stateOrdinal >= 0 && stateOrdinal < stateValues.length)
            ? stateValues[stateOrdinal]
            : State.WORKSPACE;
  ...
  /**
   * Sets the all apps button. This method is called from {@link Hotseat}. TODO: Get rid of this.
   */
  ...
  public View createShortcut(ViewGroup parent, ShortcutInfo info) {
    BubbleTextView favorite =
        (BubbleTextView)
            LayoutInflater.from(parent.getContext()).inflate(R.layout.app_icon, parent, false);
  ...
  private void completeAddShortcut(
      Intent data, long container, long screenId, int cellX, int cellY, PendingRequestArgs args) {
    ...
    if (Utilities.ATLEAST_OREO) {
      info =
          LauncherAppsCompatVO.createShortcutInfoFromPinItemRequest(
    ...
    if (info == null) {
      ...
      info =
          Process.myUserHandle().equals(args.user)
      ...
      if (info == null) {
        ...
        return;
      } else if (!new PackageManagerHelper(this)
    ...
    if (container < 0) {
      ...
      if (cellX >= 0 && cellY >= 0) {
        ...
        if (mWorkspace.createUserFolderIfNecessary(
            view, container, layout, cellXY, 0, true, null, null)) {
          return;
        }
  ...
  void completeAddAppWidget(
    ...
    getModelWriter()
        .addItemToDatabase(
            launcherInfo, itemInfo.container, itemInfo.screenId, itemInfo.cellX, itemInfo.cellY);
  ...
  protected void onNewIntent(Intent intent) {
    ...
    if (isActionMain) {
      ...
      if (topOpenView instanceof PopupContainerWithArrow) {
        ued.logActionCommand(
            Action.Command.HOME_INTENT,
            topOpenView.getExtendedTouchView(),
            ContainerType.DEEPSHORTCUTS);
      } else if (topOpenView instanceof Folder) {
        ued.logActionCommand(
            Action.Command.HOME_INTENT,
            ((Folder) topOpenView).getFolderIcon(),
    ...
    if (isActionMain) {
      boolean callbackAllowsMoveToDefaultScreen =
          mLauncherCallbacks == null || mLauncherCallbacks.shouldMoveToDefaultScreenOnHomeIntent();
  ...
  protected void onSaveInstanceState(Bundle outState) {
    if (mWorkspace.getChildCount() > 0) {
      outState.putInt(
          RUNTIME_STATE_CURRENT_SCREEN, mWorkspace.getCurrentPageOffsetFromCustomContent());
    }
  ...
  /**
   * Indicates that we want global search for this activity by setting the globalSearch argument for
   * {@link #startSearch} to true.
   */
  @Override
  ...
  public void startGlobalSearch(
      String initialQuery, boolean selectInitialQuery, Bundle appSearchData, Rect sourceBounds) {
  ...
  public void addPendingItem(
      PendingAddItemInfo info, long container, long screenId, int[] cell, int spanX, int spanY) {
  ...
  FolderIcon addFolder(
      CellLayout layout, long container, final long screenId, int cellX, int cellY) {
  ...
  public void handleBackPressed() {
    ...
    if (topView != null) {
      if (topView.getActiveTextView() != null) {
        ...
        if (topView instanceof PopupContainerWithArrow) {
          ued.logActionCommand(
              Action.Command.BACK, topView.getExtendedTouchView(), ContainerType.DEEPSHORTCUTS);
        } else if (topView instanceof Folder) {
          ued.logActionCommand(
              Action.Command.BACK, ((Folder) topView).getFolderIcon(), ContainerType.FOLDER);
        }
        topView.close(true);
      }
    ...
    if (mWorkspace.isOnOrMovingToCustomContent()) {
      mWorkspace.moveToScreenNextToCustomContent(true);
    }
  }
      ...
      if (mWorkspace.isInOverviewMode()) {
        getUserEventDispatcher()
            .logActionOnContainer(
                LauncherLogProto.Action.Type.TOUCH,
                LauncherLogProto.Action.Direction.NONE,
                LauncherLogProto.ContainerType.OVERVIEW,
                mWorkspace.getCurrentPage());
        showWorkspace(true);
      }
      return;
  ...
  public void onClickPendingWidget(final PendingAppWidgetHostView v) {
    ...
    if (v.isReadyForClickSetup()) {
      ...
      if (info.hasRestoreFlag(LauncherAppWidgetInfo.FLAG_ID_NOT_VALID)) {
        ...
        addFlowHandler.startBindFlow(this, info.appWidgetId, info, REQUEST_BIND_PENDING_APPWIDGET);
      } else {
  ...
  /**
   * Event handler for the "grid" button or "caret" that appears on the home screen, which enters
   * all apps mode. In verticalBarLayout the caret can be seen when all apps is open, and so in that
   * case reverses the action.
   *
  ...
  protected void onClickAllAppsButton(View v) {
    ...
    if (!isAppsViewVisible()) {
      ...
      showAppsView(true /* animated */, true /* updatePredictedApps */, false /* focusSearchBar */);
    } else {
      showWorkspace(true);
    }
  }
  ...
  protected void onLongClickAllAppsButton(View v) {
    ...
    if (!isAppsViewVisible()) {
      getUserEventDispatcher()
          .logActionOnControl(Action.Touch.LONGPRESS, ControlType.ALL_APPS_BUTTON);
      showAppsView(true /* animated */, true /* updatePredictedApps */, true /* focusSearchBar */);
    } else {
      showWorkspace(true);
    }
  }

  private void onClickPendingAppItem(
      final View v, final String packageName, boolean downloadStarted) {
    if (downloadStarted) {
    ...
    if ((v instanceof BubbleTextView) && shortcut.hasPromiseIconUi()) {
      ...
      if (!TextUtils.isEmpty(packageName)) {
        onClickPendingAppItem(
            v, packageName, shortcut.hasStatusFlag(ShortcutInfo.FLAG_INSTALL_SESSION_ACTIVE));
        return;
      }
    }
  ...
  private void startAppShortcutOrInfoActivity(View v) {
    ...
    boolean success = startActivitySafely(v, intent, item);
    getUserEventDispatcher()
        .logAppLaunch(v, intent, item.user); // TODO for discovered apps b/35802115
  ...
  public void onClickWallpaperPicker(View v) {
    ...
    Intent intent =
        new Intent(Intent.ACTION_SET_WALLPAPER).putExtra(Utilities.EXTRA_WALLPAPER_OFFSET, offset);
  ...
  /**
   * Called when the user stops interacting with the launcher. This implies that the user is now on
   * the homescreen and is not doing housekeeping.
   */
   ...
   * time to stop doing things that only make sense when the user is on the homescreen and not doing
   * housekeeping.
   */
  ...
  private void startShortcutIntentSafely(Intent intent, Bundle optsBundle, ItemInfo info) {
    try {
      ...
      try {
        ...
        if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT) {
          ...
          DeepShortcutManager.getInstance(this)
              .startShortcut(packageName, id, intent, optsBundle, info.user);
        } else {
      ...
      if (intent.getComponent() == null
        ...
        setWaitingForResult(
            PendingRequestArgs.forIntent(REQUEST_PERMISSION_CALL_PHONE, intent, info));
        requestPermissions(
    ...
    if (!mDragController.isDragging()) {
      if (itemUnderLongClick == null) {
        ...
        if (mWorkspace.isInOverviewMode()) {
          ...
          getUserEventDispatcher()
              .logActionOnContainer(
                  Action.Touch.LONGPRESS, Action.Direction.NONE, ContainerType.OVERVIEW);
        } else {
  ...
  public boolean showWorkspace(boolean animated, Runnable onCompleteRunnable) {
    ...
    if (changed || mAllAppsController.isTransitioning()) {
      ...
      mStateTransitionAnimation.startAnimationToWorkspace(
          mState, mWorkspace.getState(), Workspace.State.NORMAL, animated, onCompleteRunnable);
  ...
  /**
   * Shows the overview button, and if {@param requestButtonFocus} is set, will force the focus onto
   * one of the overview panel buttons.
   */
  void showOverviewMode(boolean animated, boolean requestButtonFocus) {
    ...
    mWorkspace.setVisibility(View.VISIBLE);
    mStateTransitionAnimation.startAnimationToWorkspace(
        mState, mWorkspace.getState(), Workspace.State.OVERVIEW, animated, postAnimRunnable);
    setState(State.WORKSPACE);
  ...
  public Animator startWorkspaceStateChangeAnimation(
      Workspace.State toState, boolean animated, AnimationLayerSet layerViews) {
  ...
  public void exitSpringLoadedDragMode() {
    if (mState == State.APPS_SPRING_LOADED) {
      showAppsView(
          true /* animated */, false /* updatePredictedApps */, false /* focusSearchBar */);
   ...
   * onResume.
   *
   * <p>This needs to be called from incoming places where resources might have been loaded while
   * the activity is paused. That is because the Configuration (e.g., rotation) might be wrong when
   * we're not running, and if the activity comes back to what the configuration was when we were
   * paused, activity is not restarted.
   *
   * <p>Implementation of the method from LauncherModel.Callbacks.
   *
   ...
   *
   * <p>This needs to be called from incoming places where resources might have been loaded while we
   * are paused. That is becaues the Configuration might be wrong when we're not running, and if it
   * comes back to what it was when we were paused, we are not restarted.
   *
   * <p>Implementation of the method from LauncherModel.Callbacks.
   *
    ...
    for (int i = 0; i < end; i++) {
      ...
      if (item.container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
        ...
        if (cl != null && cl.isOccupied(item.cellX, item.cellY)) {
          ...
          Object tag = v.getTag();
          String desc =
              "Collision while binding workspace item: " + item + ". Collides with " + tag;
    ...
    if (!item.hasRestoreFlag(LauncherAppWidgetInfo.FLAG_PROVIDER_NOT_READY)
      ...
      if (item.hasRestoreFlag(LauncherAppWidgetInfo.FLAG_ID_NOT_VALID)) {
        if (!item.hasRestoreFlag(LauncherAppWidgetInfo.FLAG_ID_ALLOCATED)) {
          ...
          boolean success =
              mAppWidgetManager.bindAppWidgetIdIfAllowed(item.appWidgetId, appWidgetInfo, options);
  ...
  public void finishBindingItems() {
    ...
    if (mPendingActivityResult != null) {
      handleActivityResult(
          mPendingActivityResult.requestCode,
  ...
  public void bindAllApplications(final ArrayList<AppInfo> apps) {
    Runnable r =
        new RunnableWithId(RUNNABLE_ID_BIND_APPS) {
          public void run() {
  ...
  public void bindAllWidgets(final MultiHashMap<PackageItemInfo, WidgetItem> allWidgets) {
    Runnable r =
        new RunnableWithId(RUNNABLE_ID_BIND_WIDGETS) {
          @Override
          public void run() {
  ...
  /**
   * @param packageUser if null, refreshes all widgets and shortcuts, otherwise only refreshes the
   *     widgets and shortcuts associated with the given package/user
   */
  ...
  private boolean shouldShowDiscoveryBounce() {
    ...
    return mState == State.WORKSPACE
        && !mSharedPrefs.getBoolean(APPS_VIEW_SHOWN, false)
        && !um.isDemoUser();
  }
  ...
  public void onProvideKeyboardShortcuts(
    ...
    if (new CustomActionsPopup(this, currentFocus).canShow()) {
      shortcutInfos.add(
          new KeyboardShortcutInfo(
              getString(R.string.custom_actions), KeyEvent.KEYCODE_O, KeyEvent.META_CTRL_ON));
    }
    if (currentFocus.getTag() instanceof ItemInfo
        && DeepShortcutManager.supportsShortcuts((ItemInfo) currentFocus.getTag())) {
      shortcutInfos.add(
          new KeyboardShortcutInfo(
              getString(R.string.action_deep_shortcut), KeyEvent.KEYCODE_S, KeyEvent.META_CTRL_ON));
    }
