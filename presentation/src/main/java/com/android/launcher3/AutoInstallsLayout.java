...
public class AutoInstallsLayout {
  ...
  static AutoInstallsLayout get(
    ...
    return get(
        context, customizationApkInfo.first, customizationApkInfo.second, appWidgetHost, callback);
  }
  ...
  static AutoInstallsLayout get(
    ...
    String layoutName =
        String.format(
            Locale.ENGLISH,
            FORMATTED_LAYOUT_RES_WITH_HOSTEAT,
            grid.numColumns,
            grid.numRows,
    ...
    if (layoutId == 0) {
      ...
      layoutName =
          String.format(Locale.ENGLISH, FORMATTED_LAYOUT_RES, grid.numColumns, grid.numRows);
    ...
    return new AutoInstallsLayout(
        context, appWidgetHost, callback, targetRes, layoutId, TAG_WORKSPACE);
  }
  ...
  protected void parseContainerAndScreen(XmlResourceParser parser, long[] out) {
    if (HOTSEAT_CONTAINER_NAME.equals(getAttributeValue(parser, ATTR_CONTAINER))) {
      ...
      out[1] =
          (FeatureFlags.NO_ALL_APPS_ICON || rank < mIdp.getAllAppsButtonRank()) ? rank : (rank + 1);
    } else {
  ...
  protected int parseAndAddNode(
      XmlResourceParser parser, ArrayMap<String, TagParser> tagParserMap, ArrayList<Long> screenIds)
      throws XmlPullParserException, IOException {
    ...
    mValues.put(
        Favorites.CELLX, convertToDistanceFromEnd(getAttributeValue(parser, ATTR_X), mColumnCount));
    mValues.put(
        Favorites.CELLY, convertToDistanceFromEnd(getAttributeValue(parser, ATTR_Y), mRowCount));
      ...
      if (!TextUtils.isEmpty(packageName) && !TextUtils.isEmpty(className)) {
        ...
        try {
          ...
          try {
            ...
            String[] packages =
                mPackageManager.currentToCanonicalPackageNames(new String[] {packageName});
          ...
          return addShortcut(
              info.loadLabel(mPackageManager).toString(), intent, Favorites.ITEM_TYPE_APPLICATION);
  ...
  protected class AutoInstallParser implements TagParser {
    ...
    public long parseAndAdd(XmlResourceParser parser) {
      ...
      return addShortcut(
          mContext.getString(R.string.package_state_unknown),
          intent,
  ...
  /**
   * AppWidget parser: Required attributes packageName, className, spanX and spanY. Options child
   * nodes: <extra key=... value=... /> It adds a pending widget which allows the widget to come
   * later. If there are extras, those are passed to widget options during bind. The config activity
   * for the widget (if present) is not shown, so any optional configurations should be passed as
   * extras and the widget should support reading these widget options.
   */
    ...
    public long parseAndAdd(XmlResourceParser parser) throws XmlPullParserException, IOException {
      ...
      if (folderItems.size() < 2) {
        ...
        if (folderItems.size() == 1) {
          ...
          mDb.update(Favorites.TABLE_NAME, childValues, Favorites._ID + "=" + addedId, null);
        }
      }
      return addedId;
    }
   ...
   */
  protected static int getAttributeResourceValue(
      XmlResourceParser parser, String attribute, int defaultValue) {
    int value =
