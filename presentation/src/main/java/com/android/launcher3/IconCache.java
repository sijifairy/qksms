...
public class IconCache {
  ...
  public IconCache(Context context, InvariantDeviceProfile inv) {
    ...
    mIconProvider =
        Utilities.getOverrideObject(IconProvider.class, context, R.string.icon_provider_class);
  ...
  /**
   * Updates the persistent DB, such that only entries corresponding to {@param apps} remain in the
   * DB and are updated.
   *
   * @return The set of packages for which icons have updated.
   */
  private void updateDBIcons(
      UserHandle user, List<LauncherActivityInfo> apps, Set<String> ignorePackages) {
    long userSerial = mUserManager.getSerialNumberForUser(user);
      ...
      c =
          mIconDb.query(
              new String[] {
                ...
                IconDB.COLUMN_LAST_UPDATED,
                IconDB.COLUMN_VERSION,
    ...
    if (!itemsToRemove.isEmpty()) {
      mIconDb.delete(Utilities.createDbSelectionQuery(IconDB.COLUMN_ROWID, itemsToRemove), null);
    }
   ...
   *
   * @param replaceExisting if true, it will recreate the bitmap even if it already exists in the
   *     memory. This is useful then the previous bitmap was created using old data.
   */
  @Thunk
  synchronized void addIconToDBAndMemCache(
      LauncherActivityInfo app, PackageInfo info, long userSerial, boolean replaceExisting) {
    ...
    if (entry == null) {
      ...
      entry.icon =
          LauncherIcons.createBadgedIconBitmap(
              ...
              app.getUser(),
              mContext,
              app.getApplicationInfo().targetSdkVersion);
    }
    ...
    ContentValues values =
        newContentValues(
            entry.icon, lowResIcon, entry.title.toString(), app.getApplicationInfo().packageName);
   ...
   */
  public IconLoadRequest updateIconInBackground(
      final ItemInfoUpdateReceiver caller, final ItemInfoWithIcon info) {
    Runnable request =
        new Runnable() {
  ...
  public synchronized void getTitleAndIcon(
      ItemInfoWithIcon info, LauncherActivityInfo activityInfo, boolean useLowResIcon) {
  ...
  public synchronized void getTitleAndIcon(ItemInfoWithIcon info, boolean useLowResIcon) {
    ...
    if (info.getTargetComponent() == null) {
      ...
      getTitleAndIcon(
          info, new ActivityInfoProvider(info.getIntent(), info.user), true, useLowResIcon);
    }
  }
  ...
  public synchronized void getTitleAndIconForApp(PackageItemInfo infoInOut, boolean useLowResIcon) {
    CacheEntry entry =
        getEntryForPackageLocked(infoInOut.packageName, infoInOut.user, useLowResIcon);
    applyCacheEntry(entry, infoInOut);
  }
        ...
        if (info != null) {
          ...
          if (usePackageIcon) {
            CacheEntry packageEntry =
                getEntryForPackageLocked(componentName.getPackageName(), user, false);
  ...
  /**
   * Adds a default package entry in the cache. This entry is not persisted and will be removed when
   * the cache is flushed.
   */
  public synchronized void cachePackageInstallInfo(
      String packageName, UserHandle user, Bitmap icon, CharSequence title) {
    removeFromMemCacheLocked(packageName, user);
  ...
  private CacheEntry getEntryForPackageLocked(
      String packageName, UserHandle user, boolean useLowResIcon) {
    Preconditions.assertWorkerThread();
    ComponentKey cacheKey = getPackageKey(packageName, user);
    CacheEntry entry = mCache.get(cacheKey);
    ...
    if (entry == null || (entry.isLowResIcon && !useLowResIcon)) {
      ...
      if (!getEntryFromDB(cacheKey, entry, useLowResIcon)) {
        try {
          int flags =
              Process.myUserHandle().equals(user) ? 0 : PackageManager.GET_UNINSTALLED_PACKAGES;
          ...
          if (mInstantAppResolver.isInstantApp(appInfo)) {
            icon =
                LauncherIcons.badgeWithDrawable(
                    icon, mContext.getDrawable(R.drawable.ic_instant_app_badge), mContext);
          }
  ...
  private static final class IconDB extends SQLiteCacheHelper {
    ...
    private static final int RELEASE_VERSION =
        DB_VERSION + (FeatureFlags.LAUNCHER3_DISABLE_ICON_NORMALIZATION ? 0 : 1);
    ...
    public IconDB(Context context, int iconPixelSize) {
      super(
          context, LauncherFiles.APP_ICONS_DB, (RELEASE_VERSION << 16) + iconPixelSize, TABLE_NAME);
    }
  ...
  private ContentValues newContentValues(
      Bitmap icon, Bitmap lowResIcon, String label, String packageName) {
  ...
  private Bitmap generateLowResIcon(Bitmap icon) {
    return Bitmap.createScaledBitmap(
        ...
        icon.getHeight() / LOW_RES_SCALE_FACTOR,
        true);
  }
