...
public class WidgetPreviewLoader {
  ...
  /**
   * Weak reference objects, do not prevent their referents from being made finalizable, finalized,
   * and then reclaimed. Note: synchronized block used for this variable is expensive and the block
   * should always be posted to a background thread.
   */
  @Thunk
  ...
  /**
   * Generates the widget preview on {@link AsyncTask#THREAD_POOL_EXECUTOR}. Must be called on UI
   * thread
   *
   * @return a request id which can be used to cancel the request.
   */
  public CancellationSignal getPreview(
      WidgetItem item, int previewWidth, int previewHeight, WidgetCell caller, boolean animate) {
    String size = previewWidth + "x" + previewHeight;
    WidgetCacheKey key = new WidgetCacheKey(item.componentName, item.user, size);

    PreviewLoadTask task =
        new PreviewLoadTask(key, item, previewWidth, previewHeight, caller, animate);
  ...
  /**
   * Updates the persistent DB: 1. Any preview generated for an old package version is removed 2.
   * Any preview for an absent package is removed This ensures that we remove entries for packages
   * which changed while the launcher was dead.
   *
   * @param packageUser if provided, specifies that list only contains previews for the given
   *     package/user, otherwise the list contains all previews
   */
  public void removeObsoletePreviews(
      ArrayList<? extends ComponentKey> list, @Nullable PackageUserKey packageUser) {
    ...
    long passedUserId =
        packageUser == null ? 0 : mUserManager.getSerialNumberForUser(packageUser.mUser);
    Cursor c = null;
    try {
      c =
          mDb.query(
              new String[] {
                CacheDb.COLUMN_USER,
                CacheDb.COLUMN_PACKAGE,
                CacheDb.COLUMN_LAST_UPDATED,
                CacheDb.COLUMN_VERSION
              },
              null,
              null);
      while (c.moveToNext()) {
        ...
        if (packageUser != null
            && (!pkg.equals(packageUser.mPackageName) || userId != passedUserId)) {
  ...
  private Bitmap generatePreview(
      BaseActivity launcher, WidgetItem item, Bitmap recycle, int previewWidth, int previewHeight) {
    if (item.widgetInfo != null) {
      return generateWidgetPreview(launcher, item.widgetInfo, previewWidth, recycle, null);
    } else {
      return generateShortcutPreview(
          launcher, item.activityInfo, previewWidth, previewHeight, recycle);
    }
  }

  /**
   * Generates the widget preview from either the {@link AppWidgetManagerCompat} or cache and add
   * badge at the bottom right corner.
   *
   * @param launcher
  ...
  public Bitmap generateWidgetPreview(
    ...
    if (widgetPreviewExists) {
      ...
      p.setStrokeWidth(
          mContext.getResources().getDimension(R.dimen.widget_preview_cell_divider_width));
      ...
      try {
        ...
        if (icon != null) {
          ...
          int iconSize =
              (int) Math.min(appIconSize * scale, Math.min(boxRect.width(), boxRect.height()));
  ...
  private Bitmap generateShortcutPreview(
      BaseActivity launcher,
      ShortcutConfigActivityInfo info,
      int maxWidth,
      int maxHeight,
      Bitmap preview) {
    int iconSize = launcher.getDeviceProfile().iconSizePx;
    int padding =
        launcher.getResources().getDimensionPixelSize(R.dimen.widget_preview_shortcut_padding);
  ...
  public class PreviewLoadTask extends AsyncTask<Void, Void, Bitmap>
    ...
    PreviewLoadTask(
      ...
      if (DEBUG) {
        Log.d(TAG, String.format("%s, %s, %d, %d", mKey, mInfo, mPreviewHeight, mPreviewWidth));
      }
    }
    ...
    protected Bitmap doInBackground(Void... params) {
      ...
      if (!isCancelled() && preview == null) {
        ...
        mVersions = persistable ? getPackageVersion(mKey.componentName.getPackageName()) : null;
