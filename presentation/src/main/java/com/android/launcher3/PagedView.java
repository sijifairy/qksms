...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
  ...
  private int validateNewPage(int newPage) {
    ...
    if (mFreeScroll) {
      ...
      validatedPage =
          Math.max(mTempVisiblePagesRange[0], Math.min(newPage, mTempVisiblePagesRange[1]));
    }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int maxSize =
        Math.max(
            dm.widthPixels + mInsets.left + mInsets.right,
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    for (int i = startIndex; i != endIndex; i += delta) {
      ...
      if (child.getVisibility() != View.GONE) {
        ...
        child.layout(
            childLeft, childTop, childLeft + child.getMeasuredWidth(), childTop + childHeight);
  ...
  private boolean isTouchPointInViewportWithBuffer(int x, int y) {
    sTmpRect.set(
        mViewport.left - mViewport.width() / 2,
        mViewport.top,
        mViewport.right + mViewport.width() / 2,
        mViewport.bottom);
    ...
    switch (action & MotionEvent.ACTION_MASK) {
      ...
      case MotionEvent.ACTION_UP:
        if (mTouchState == TOUCH_STATE_SCROLLING) {
          ...
          boolean isFling =
              mTotalMotionX > MIN_LENGTH_FOR_FLING && shouldFlingForVelocity(velocityX);
          ...
          if (!mFreeScroll) {
            ...
            mScroller.setInterpolator(mDefaultInterpolator);
            mScroller.fling(
                initialScrollX, getScrollY(), vX, 0, Integer.MIN_VALUE, Integer.MAX_VALUE, 0, 0);
  ...
  protected void snapToPage(
      int whichPage, int duration, boolean immediate, TimeInterpolator interpolator) {
    whichPage = validateNewPage(whichPage);
  ...
  protected void snapToPage(
      int whichPage, int delta, int duration, boolean immediate, TimeInterpolator interpolator) {
    whichPage = validateNewPage(whichPage);
    ...
    if (mDragView != null) {
      Animator anim =
          LauncherAnimUtils.ofPropertyValuesHolder(
                  mDragView,
                  new PropertyListBuilder().scale(1).translationX(0).translationY(0).build())
