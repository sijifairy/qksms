...
public class CellLayout extends ViewGroup implements BubbleTextShadowHandler {
  ...
  public void setPressedIcon(BubbleTextView icon, Bitmap background) {
    if (icon == null || background == null) {
      ...
      if (mTouchFeedbackView.setBitmap(background)) {
        mTouchFeedbackView.alignWithIconView(
            icon, mShortcutsAndWidgets, null /* clipAgainstView */);
  ...
  void setIsDragOverlapping(boolean isDragOverlapping) {
    if (mIsDragOverlapping != isDragOverlapping) {
      ...
      mBackground.setState(mIsDragOverlapping ? BACKGROUND_STATE_ACTIVE : BACKGROUND_STATE_DEFAULT);
      invalidate();
    }
  }
  ...
  private ParcelableSparseArray getJailedArray(SparseArray<Parcelable> container) {
    ...
    return parcelable instanceof ParcelableSparseArray
        ? (ParcelableSparseArray) parcelable
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mFolderLeaveBehind.delegateCellX >= 0 && mFolderLeaveBehind.delegateCellY >= 0) {
      cellToPoint(
          mFolderLeaveBehind.delegateCellX, mFolderLeaveBehind.delegateCellY, mTempLocation);
      canvas.save();
      canvas.translate(mTempLocation[0], mTempLocation[1]);
  ...
  public void setFolderLeaveBehindCell(int x, int y) {
    ...
    mFolderLeaveBehind.setup(mLauncher, null, child.getMeasuredWidth(), child.getPaddingTop());
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    mTouchFeedbackView.measure(
        MeasureSpec.makeMeasureSpec(
            mCellWidth + mTouchFeedbackView.getExtraSize(), MeasureSpec.EXACTLY),
        MeasureSpec.makeMeasureSpec(
            mCellHeight + mTouchFeedbackView.getExtraSize(), MeasureSpec.EXACTLY));
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    boolean isFullscreen =
        mShortcutsAndWidgets.getChildCount() > 0
  ...
  void visualizeDropLocation(
      View v,
      DragPreviewProvider outlineProvider,
      int cellX,
      int cellY,
      int spanX,
      int spanY,
  ...
  public String getItemMoveDescription(int cellX, int cellY) {
    if (mContainerType == HOTSEAT) {
      return getContext().getString(R.string.move_to_hotseat_position, Math.max(cellX, cellY) + 1);
    } else {
      return getContext().getString(R.string.move_to_empty_cell, cellY + 1, cellX + 1);
    }
  }
  ...
  int[] findNearestVacantArea(
    ...
    return findNearestArea(
        pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, true, result, resultSpan);
  }
  ...
  /**
   * Find a vacant area that will fit the given bounds nearest the requested cell location, and will
   * also weigh in a suggested direction vector of the desired location. This method computers
   * distance based on unit grid distances, not pixel distances.
   *
  ...
  private boolean addViewToTempLocation(
      View v, Rect rectOccupiedByPotentialDrop, int[] direction, ItemConfiguration currentState) {
    ...
    findNearestArea(
        c.cellX, c.cellY, c.spanX, c.spanY, direction, mTmpOccupied.cells, null, mTempLocation);
   ...
   * cluster and determining how those edges interact with other views. The edges essentially define
   * a fine-grained boundary around the cluster of views -- like a more precise version of a
   * bounding box.
   */
  ...
  private boolean rearrangementExists(
    ...
    if (attemptPushInDirection(
        mIntersectingViews, mOccupiedRect, direction, ignoreView, solution)) {
      return true;
    }
  ...
  private ItemConfiguration findReorderSolution(
      int pixelX,
      int pixelY,
      int minSpanX,
      int minSpanY,
    ...
    success =
        rearrangementExists(result[0], result[1], spanX, spanY, direction, dragView, solution);
    ...
    if (!success) {
      ...
      if (spanX > minSpanX && (minSpanY == spanY || decX)) {
        return findReorderSolution(
            pixelX, pixelY, minSpanX, minSpanY, spanX - 1, spanY, direction, dragView, false,
            solution);
      } else if (spanY > minSpanY) {
        return findReorderSolution(
            pixelX, pixelY, minSpanX, minSpanY, spanX, spanY - 1, direction, dragView, true,
            solution);
      }
  ...
  private void animateItemsToSolution(
      ItemConfiguration solution, View dragView, boolean commitDragView) {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (c != null) {
        animateChildToPosition(
            child, c.cellX, c.cellY, REORDER_ANIMATION_DURATION, 0, DESTRUCTIVE_REORDER, false);
  ...
  private void beginOrAdjustReorderPreviewAnimations(
      ItemConfiguration solution, View dragView, int delay, int mode) {
    int childCount = mShortcutsAndWidgets.getChildCount();
    for (int i = 0; i < childCount; i++) {
      View child = mShortcutsAndWidgets.getChildAt(i);
      if (child == dragView) continue;
      CellAndSpan c = solution.map.get(child);
      boolean skip =
          mode == ReorderPreviewAnimation.MODE_HINT
              && solution.intersectingViews != null
              && !solution.intersectingViews.contains(child);

      LayoutParams lp = (LayoutParams) child.getLayoutParams();
      if (c != null && !skip) {
        ReorderPreviewAnimation rha =
            new ReorderPreviewAnimation(
                child, mode, lp.cellX, lp.cellY, c.cellX, c.cellY, c.spanX, c.spanY);
        rha.animate();
      }
    }
  }
  ...
  class ReorderPreviewAnimation {
    ...
    public ReorderPreviewAnimation(
        View child,
        int mode,
        int cellX0,
        int cellY0,
    ...
    void completeAnimationImmediately() {
      ...
      setInitialAnimationValues(true);
      a =
          LauncherAnimUtils.ofPropertyValuesHolder(
                  child,
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (info != null) {
        final boolean requiresDbUpdate =
            (info.cellX != lp.tmpCellX
                || info.cellY != lp.tmpCellY
        ...
        if (requiresDbUpdate) {
          mLauncher
              ...
              .modifyItemInDatabase(
                  info, container, screenId, info.cellX, info.cellY, info.spanX, info.spanY);
        }
      }
    }
  }
  ...
  */
  private void getDirectionVectorForDrop(
    ...
    regionToRect(
        dropRegionRect.left,
        dropRegionRect.top,
        dropRegionRect.width(),
        dropRegionRect.height(),
        dropRegionRect);
  ...
  boolean isNearestDropLocationOccupied(
      int pixelX, int pixelY, int spanX, int spanY, View dragView, int[] result) {
    result = findNearestArea(pixelX, pixelY, spanX, spanY, result);
    getViewsIntersectingRegion(
        result[0], result[1], spanX, spanY, dragView, null, mIntersectingViews);
    ...
    if (isItemPlacementDirty() && !DESTRUCTIVE_REORDER) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (lp.tmpCellX != lp.cellX || lp.tmpCellY != lp.cellY) {
          ...
          animateChildToPosition(
              child, lp.cellX, lp.cellY, REORDER_ANIMATION_DURATION, 0, false, false);
        }
      }
  ...
  boolean createAreaForResize(
      int cellX, int cellY, int spanX, int spanY, View dragView, int[] direction, boolean commit) {
  ...
  int[] performReorder(
    ...
    ItemConfiguration noShuffleSolution =
        findConfigurationNoShuffle(
            pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, dragView, new ItemConfiguration());
    ...
    if (mode == MODE_SHOW_REORDER_HINT) {
      if (finalSolution != null) {
        beginOrAdjustReorderPreviewAnimations(
            finalSolution, dragView, 0, ReorderPreviewAnimation.MODE_HINT);
  ...
  /**
   * Finds the upper-left coordinate of the first rectangle in the grid that can hold a cell of the
   * specified dimensions. If intersectX and intersectY are not -1, then this method will only
  ...
  /**
   * Mark a child as having been dropped. At the beginning of the drag operation, the child may have
   * been on another screen, but it is re-parented before this method is called.
   *
  ...
  public static class LayoutParams extends ViewGroup.MarginLayoutParams {
    ...
    /**
     * Indicates whether the item will set its x, y, width and height parameters freely, or whether
     * these will be computed based on cellX, cellY, cellHSpan and cellVSpan.
     */
     ...
     * AllApps button and QSB place holder.
     */
    ...
    /**
     * Use this method, as opposed to {@link #setup(int, int, boolean, int)}, if the view needs to
     * be scaled.
     *
     * <p>ie. In multi-window mode, we setup widgets so that they are measured and laid out using
     * their full/invariant device profile sizes.
     */
    public void setup(
        int cellWidth,
        int cellHeight,
  ...
  /**
   * Returns whether an item can be placed in this CellLayout (after rearranging and/or resizing if
   * necessary).
   */
