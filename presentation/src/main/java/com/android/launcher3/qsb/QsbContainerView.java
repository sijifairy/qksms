...
/**
 * A frame layout which contains a QSB. This internally uses fragment to bind the view, which allows
 * it to contain the logic for {@link Fragment#startActivityForResult(Intent, int)}.
 *
 * <p>Note: AppWidgetManagerCompat can be disabled using FeatureFlags. In QSB, we should use
    ...
    private View createQsb(ViewGroup container) {
      ...
      if (isWidgetBound) {
        ...
        if (!Utilities.containsAll(
            AppWidgetManager.getInstance(activity).getAppWidgetOptions(widgetId), opts)) {
