...
public class PopupContainerWithArrow extends AbstractFloatingView
  ...
  @IntDef(
      flag = true,
      value = {ROUNDED_TOP_CORNERS, ROUNDED_BOTTOM_CORNERS})
  ...
  public static PopupContainerWithArrow showForIcon(BubbleTextView icon) {
    ...
    List<NotificationKeyData> notificationKeys =
        popupDataProvider.getNotificationKeysForItem(itemInfo);
    List<SystemShortcut> systemShortcuts =
        popupDataProvider.getEnabledSystemShortcutsForItem(itemInfo);
  ...
  public void populateAndShow(
      final BubbleTextView originalIcon,
    ...
    final int arrowVerticalOffset =
        resources.getDimensionPixelSize(R.dimen.popup_arrow_vertical_offset);

    mOriginalIcon = originalIcon;

    // Add dummy views first, and populate with real info when ready.
    PopupPopulator.Item[] itemsToPopulate =
        PopupPopulator.getItemsToPopulate(shortcutIds, notificationKeys, systemShortcuts);
    ...
    List<DeepShortcutView> shortcutViews =
        mShortcutsItemView == null
    ...
    if (numNotifications == 0) {
      setContentDescription(
          getContext()
              .getString(
                  R.string.shortcuts_menu_description,
                  numShortcuts,
    ...
    // Add the arrow.
    final int arrowHorizontalOffset =
  ...
  private void addDummyViews(PopupPopulator.Item[] itemTypesToPopulate, int numNotifications) {
    ...
    for (int i = 0; i < numItems; i++) {
      ...
      PopupPopulator.Item prevItemTypeToPopulate = i > 0 ? itemTypesToPopulate[i - 1] : null;
      ...
      boolean shouldUnroundTopCorners =
          prevItemTypeToPopulate != null
              && itemTypeToPopulate.isShortcut ^ prevItemTypeToPopulate.isShortcut;
      boolean shouldUnroundBottomCorners =
          nextItemTypeToPopulate != null
        ...
        int footerHeight =
            res.getDimensionPixelSize(
                notificationFooterHasIcons
                    ? R.dimen.notification_footer_height
      ...
      if (itemTypeToPopulate.isShortcut) {
        if (mShortcutsItemView == null) {
          mShortcutsItemView =
              (ShortcutsItemView) inflater.inflate(R.layout.shortcuts_item, this, false);
        ...
        if (itemTypeToPopulate != PopupPopulator.Item.SYSTEM_SHORTCUT_ICON
          ...
          item.getLayoutParams().height =
              res.getDimensionPixelSize(R.dimen.bg_popup_item_condensed_height);
  ...
  private void animateOpen() {
    ...
    final ValueAnimator revealAnim =
        new RoundedRectRevealOutlineProvider(radius, radius, mStartRect, mEndRect)
  ...
  private void enforceContainedWithinScreen(int left, int right) {
    ...
    if (getTranslationX() + left < 0 || getTranslationX() + right > dragLayer.getWidth()) {
    ...
    int arrowCenterX =
        getResources()
            .getDimensionPixelSize(
                mIsLeftAligned ^ mIsRtl
                    ? R.dimen.popup_arrow_horizontal_center_start
                    : R.dimen.popup_arrow_horizontal_center_end);
    if (!mIsLeftAligned) {
      arrowCenterX = getMeasuredWidth() - arrowCenterX;
    }
    int arrowHeight = getMeasuredHeight() - getPaddingTop() - getPaddingBottom() - itemsTotalHeight;
   ...
   *
   * <p>These are the preferred orientations, in order (RTL prefers right-aligned over left): -
   * Above and left-aligned - Above and right-aligned - Below and left-aligned - Below and
   * right-aligned
   *
   * <p>So we always align left if there is enough horizontal space and align above if there is
  ...
  private void orientAboutIcon(BubbleTextView icon, int arrowHeight) {
    ...
    int x = leftAlignedX;
    boolean canBeLeftAligned =
        leftAlignedX + width + insets.left < dragLayer.getRight() - insets.right;
    ...
    int iconHeight =
        icon.getIcon() != null ? icon.getIcon().getBounds().height() : icon.getHeight();
   ...
   *
   * @param horizontalOffset the horizontal offset of the arrow, so that it points at the center of
   *     the original icon
   */
  private View addArrowView(int horizontalOffset, int verticalOffset, int width, int height) {
    ...
    if (Gravity.isVertical(mGravity)) {
      ...
      ShapeDrawable arrowDrawable =
          new ShapeDrawable(TriangleShape.create(width, height, !mIsAboveIcon));
      ...
      PopupItemView itemAttachedToArrow =
          (PopupItemView) (getChildAt(mIsAboveIcon ? getChildCount() - 1 : 0));
   ...
   *
   * <p>Current behavior: - Start the drag if the touch passes a certain distance from the original
   * touch down.
   */
    ...
    if (badgeInfo == null || badgeInfo.getNotificationKeys().size() == 0) {
      ...
      if (mShortcutsItemView != null) {
        ...
        mShortcutsItemView.setBackgroundWithCorners(
            backgroundColor, ROUNDED_TOP_CORNERS | ROUNDED_BOTTOM_CORNERS);
      ...
      removeNotification.play(
          adjustItemHeights(
              mNotificationItemView.getHeightMinusFooter(), hiddenShortcutsHeight, duration));
      Animator fade = ObjectAnimator.ofFloat(mNotificationItemView, ALPHA, 0).setDuration(duration);
      fade.addListener(
          new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
      ...
      final long arrowScaleDuration =
          getResources().getInteger(R.integer.config_popupArrowOpenDuration);
   ...
   */
  public Animator adjustItemHeights(
      int notificationHeightToRemove, int shortcutHeightToAdd, int duration) {
    if (mReduceHeightAnimatorSet != null) {
      mReduceHeightAnimatorSet.cancel();
    }
    final int translateYBy =
        mIsAboveIcon
            ? notificationHeightToRemove - shortcutHeightToAdd
    ...
    mReduceHeightAnimatorSet.play(
        mNotificationItemView.animateHeightRemoval(
    ...
    for (int i = 0; i < getItemCount(); i++) {
      ...
      ValueAnimator translateItem =
          ObjectAnimator.ofFloat(itemView, TRANSLATION_Y, itemView.getTranslationY() + translateYBy)
    ...
    mReduceHeightAnimatorSet.addListener(
        new AnimatorListenerAdapter() {
          @Override
          public void onAnimationEnd(Animator animation) {
            if (mIsAboveIcon) {
  ...
  public void onDropCompleted(
      View target, DropTarget.DragObject d, boolean isFlingToDelete, boolean success) {
    if (!success) {
      d.dragView.remove();
