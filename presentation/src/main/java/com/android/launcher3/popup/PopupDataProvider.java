...
public class PopupDataProvider implements NotificationListener.NotificationsChangedListener {
    ...
    mSystemShortcuts =
        new SystemShortcut[] {
          Utilities.getOverrideObject(
              SystemShortcut.Custom.class, launcher, R.string.custom_shortcut_class),
  ...
  public void onNotificationPosted(
    ...
    updateLauncherIconBadges(
        Utilities.singletonHashSet(postedPackageUserKey), badgeShouldBeRefreshed);
  }
   ...
   * @param shouldRefresh An optional parameter that will allow us to only refresh badges that have
   *     actually changed. If a notification updated its content but not its count or icon, then the
  ...
  /**
   * Determines whether the badge should show a notification icon rather than a number, and sets
   * that icon on the BadgeInfo if so.
   *
    ...
    if (notificationListener != null && badgeInfo.getNotificationKeys().size() >= 1) {
      ...
      for (NotificationKeyData notificationKeyData : badgeInfo.getNotificationKeys()) {
        ...
        StatusBarNotification[] activeNotifications =
            notificationListener.getActiveNotifications(new String[] {notificationKey});
