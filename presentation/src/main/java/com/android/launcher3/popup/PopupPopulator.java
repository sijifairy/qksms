...
/**
 * Contains logic relevant to populating a {@link PopupContainerWithArrow}. In particular, this
 * class determines which items appear in the container, and in what order.
 */
public class PopupPopulator {
  ...
  public static @NonNull Item[] getItemsToPopulate(
    ...
    int numItems =
        Math.min(MAX_SHORTCUTS, numShortcuts) + numNotificationItems + systemShortcuts.size();
  ...
  /**
   * Filters the shortcuts so that only MAX_SHORTCUTS or fewer shortcuts are retained. We want the
   * filter to include both static and dynamic shortcuts, so we always include NUM_DYNAMIC dynamic
   * shortcuts, if at least that many are present.
   *
      ...
      public void run() {
        if (notificationView != null) {
          List<StatusBarNotification> notifications =
              launcher.getPopupDataProvider().getStatusBarNotificationsForKeys(notificationKeys);
        ...
        List<ShortcutInfoCompat> shortcuts =
            DeepShortcutManager.getInstance(launcher)
        ...
        for (int i = 0; i < shortcuts.size() && i < shortcutViews.size(); i++) {
          ...
          si.iconBitmap = LauncherIcons.createShortcutIcon(shortcut, launcher, false /* badged */);
          si.rank = i;
          uiHandler.post(new UpdateShortcutChild(container, shortcutViews.get(i), si, shortcut));
        }
        ...
        for (int i = 0; i < systemShortcuts.size(); i++) {
          ...
          uiHandler.post(
              new UpdateSystemShortcutChild(
                  container, systemShortcutViews.get(i), systemShortcut, launcher, originalInfo));
        }
        uiHandler.post(
            new Runnable() {
              @Override
  ...
  private static class UpdateShortcutChild implements Runnable {
    ...
    public UpdateShortcutChild(
        PopupContainerWithArrow container,
        DeepShortcutView shortcutChild,
        ShortcutInfo shortcutChildInfo,
  ...
  private static class UpdateNotificationChild implements Runnable {
    ...
    public UpdateNotificationChild(
        NotificationItemView notificationView, List<NotificationInfo> notificationInfos) {
  ...
  private static class UpdateSystemShortcutChild implements Runnable {
    ...
    public UpdateSystemShortcutChild(
        PopupContainerWithArrow container,
        View systemShortcutChild,
        SystemShortcut systemShortcut,
        Launcher launcher,
        ItemInfo originalInfo) {
      mContainer = container;
    ...
    public void run() {
      ...
      mSystemShortcutChild.setOnClickListener(
          mSystemShortcutInfo.getOnClickListener(mLauncher, mItemInfo));
    }
  }
