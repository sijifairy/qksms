...
public class FolderPagedView extends PagedView {
  ...
  /**
   * Calculates the grid size such that {@param count} items can fit in the grid. The grid size is
   * calculated such that countY <= countX and countX = ceil(sqrt(count)) while maintaining the
   * restrictions of {@link #mMaxCountX} &amp; {@link #mMaxCountY}.
   */
  ...
  public void setupContentDimensions(int count) {
    ...
    calculateGridSize(
        count, mGridCountX, mGridCountY, mMaxCountX, mMaxCountY, mMaxItemsPerPage, sTmpArray);
  ...
  /**
   * Create space for a new item at the end, and returns the rank for that item. Also sets the
  ...
  /**
   * Adds the {@param view} to the layout based on {@param rank} and updated the position related
   * attributes. It assumes that {@param item} is already attached to the view.
   */
  ...
  public View createNewView(ShortcutInfo item) {
    final BubbleTextView textView =
        (BubbleTextView) mInflater.inflate(R.layout.folder_application, null, false);
  ...
  /**
   * Updates position and rank of all the children in the view. It essentially removes all views
   * from all the pages and then adds them again in appropriate page.
   *
  ...
  private void arrangeChildren(ArrayList<View> list, int itemCount, boolean saveChanges) {
        ...
        if (info.cellX != newX || info.cellY != newY || info.rank != rank) {
          ...
          if (saveChanges) {
            mFolder
                .mLauncher
                .getModelWriter()
                .addOrMoveItemInDatabase(info, mFolder.mInfo.id, 0, info.cellX, info.cellY);
          }
        }
        lp.cellX = info.cellX;
        lp.cellY = info.cellY;
    ...
    mFolder.mFolderName.setGravity(
        getPageCount() > 1 ? (mIsRtl ? Gravity.RIGHT : Gravity.LEFT) : Gravity.CENTER_HORIZONTAL);
  }

  public int getDesiredWidth() {
    return getPageCount() > 0
        ? (getPageAt(0).getDesiredWidth() + getPaddingLeft() + getPaddingRight())
        : 0;
  }

  public int getDesiredHeight() {
    return getPageCount() > 0
        ? (getPageAt(0).getDesiredHeight() + getPaddingTop() + getPaddingBottom())
        : 0;
  }
  ...
  public void showScrollHint(int direction) {
    float fraction =
        (direction == Folder.SCROLL_LEFT) ^ mIsRtl ? -SCROLL_HINT_FRACTION : SCROLL_HINT_FRACTION;
  ...
  public void realTimeReorder(int empty, int target) {
    ...
    for (int i = startPos; i != endPos; i += direction) {
      ...
      if (page.animateChildToPosition(
          v, i % mGridCountX, i / mGridCountX, REORDER_ANIMATION_DURATION, delay, true, true)) {
        delay += delayAmount;
