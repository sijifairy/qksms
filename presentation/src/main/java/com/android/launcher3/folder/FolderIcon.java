...
public class FolderIcon extends FrameLayout implements FolderListener {
  ...
  public static FolderIcon fromXml(
      int resId, Launcher launcher, ViewGroup group, FolderInfo folderInfo) {
    ...
    FolderIcon icon =
        (FolderIcon) LayoutInflater.from(group.getContext()).inflate(resId, group, false);
  ...
  private void onDrop(
    ...
    if (animateView != null) {
      ...
      if (index >= mPreviewLayoutRule.maxNumItems() && mPreviewLayoutRule.hasEnterExitIndices()) {
  ...
  /**
   * Sets mBadgeScale to 1 or 0, animating if wasBadged or isBadged is false (the badge is being
   * added or removed).
   */
  ...
  private float getLocalCenterForIndex(int index, int curNumItems, int[] center) {
    mTmpParams =
        mPreviewItemManager.computePreviewItemDrawingParams(
  ...
  public void drawBadge(Canvas canvas) {
    if ((mBadgeInfo != null && mBadgeInfo.hasBadge()) || mBadgeScale > 0) {
      ...
      mBadgeRenderer.draw(
          canvas, badgePalette, mBadgeInfo, mTempBounds, badgeScale, mTempSpaceForBadgeOffset);
    }
  }
  ...
  /**
   * This method draws the FolderIcon to an ImageView and then adds and positions that ImageView in
   * the DragLayer in the exact absolute location of the original FolderIcon.
   */
  ...
  interface PreviewLayoutRule {
    PreviewItemDrawingParams computePreviewItemDrawingParams(
