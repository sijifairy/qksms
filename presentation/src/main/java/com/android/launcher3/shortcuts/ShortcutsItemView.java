...
/**
 * A {@link PopupItemView} that contains all of the {@link DeepShortcutView}s for an app, as well as
 * the system shortcuts such as Widgets and App Info.
 */
public class ShortcutsItemView extends PopupItemView
    implements View.OnLongClickListener, View.OnTouchListener, LogContainerProvider {
  ...
  public boolean onLongClick(View v) {
    ...
    DragView dv =
        mLauncher
            ...
            .beginDragShared(
                sv.getIconView(),
                (PopupContainerWithArrow) getParent(),
                sv.getFinalInfo(),
  ...
  private void addShortcutView(View shortcutView, PopupPopulator.Item shortcutType, int index) {
    ...
    if (shortcutType == PopupPopulator.Item.SYSTEM_SHORTCUT_ICON) {
      ...
      if (mSystemShortcutIcons == null) {
        mSystemShortcutIcons =
            (LinearLayout)
                mLauncher
                    .getLayoutInflater()
                    .inflate(R.layout.system_shortcut_icons, mContent, false);
  ...
  /**
   * Hides shortcuts until only {@param maxShortcuts} are showing. Also sets {@link
   * #mHiddenShortcutsHeight} to be the amount of extra space that shortcuts will require when
   * {@link #showAllShortcuts(boolean)} is called.
   */
  ...
  /**
   * Sets all shortcuts in {@link #mShortcutsLayout} to VISIBLE, then creates an animation to reveal
   * the newly shown shortcuts.
   *
  ...
  public Animator showAllShortcuts(boolean showFromTop) {
    ...
    if (showFromTop) {
      ...
      animation.play(
          new RoundedRectRevealOutlineProvider(
                  getBackgroundRadius(),
                  getBackgroundRadius(),
                  backgroundStartRect,
                  backgroundEndRect,
    ...
    for (int i = 0; i < numShortcuts; i++) {
      ...
      animation.play(
          LauncherAnimUtils.ofPropertyValuesHolder(
              shortcut.getIconView(), new PropertyListBuilder().scale(1f).build()));
    }
    return animation;
  }
  ...
  public void enableWidgetsIfExist(final BubbleTextView originalIcon) {
    ...
    final PopupPopulator.Item widgetsItem =
        mSystemShortcutIcons == null
  ...
  @Override
  public void fillInLogContainerData(
      View v, ItemInfo info, LauncherLogProto.Target target, LauncherLogProto.Target targetParent) {
