...
/**
 * A {@link android.widget.FrameLayout} that contains a {@link DeepShortcutView}. This lets us
 * animate the DeepShortcutView (icon and text) separately from the background.
 */
public class DeepShortcutView extends FrameLayout {
  ...
  /** package private * */
  public void applyShortcutInfo(
      ShortcutInfo info, ShortcutInfoCompat detail, ShortcutsItemView container) {
    mInfo = info;
    mDetail = detail;
    ...
    int availableWidth =
        mBubbleText.getWidth()
            - mBubbleText.getTotalPaddingLeft()
            - mBubbleText.getTotalPaddingRight();
    boolean usingLongLabel =
        !TextUtils.isEmpty(longLabel)
  ...
  public ShortcutInfo getFinalInfo() {
    ...
    Launcher.getLauncher(getContext()).getModel().updateAndBindShortcutInfo(badged, mDetail);
    return badged;
  }
