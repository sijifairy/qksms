...
public class ShortcutInfoCompatBackport extends ShortcutInfoCompat {
  ...
  public ShortcutInfoCompatBackport(
      Context context,
      Resources resources,
      String packageName,
      ComponentName activity,
      XmlResourceParser parseXml)
    ...
    mEnabled =
        !xmlData.containsKey("enabled") || xmlData.get("enabled").toLowerCase().equals("true");

    if (xmlData.containsKey("icon")) {
      String icon = xmlData.get("icon");
      int resId = resources.getIdentifier(icon, null, packageName);
      mIcon = resId == 0 ? Integer.parseInt(icon.substring(1)) : resId;
    } else {
      mIcon = 0;
    }

    mShortLabel =
        xmlData.containsKey("shortcutShortLabel")
            ? resources.getString(Integer.valueOf(xmlData.get("shortcutShortLabel").substring(1)))
            : "";

    mLongLabel =
        xmlData.containsKey("shortcutLongLabel")
            ? resources.getString(Integer.valueOf(xmlData.get("shortcutLongLabel").substring(1)))
            : mShortLabel;

    mDisabledMessage =
        xmlData.containsKey("shortcutDisabledMessage")
            ? resources.getString(
                Integer.valueOf(xmlData.get("shortcutDisabledMessage").substring(1)))
            : "";
    ...
    String action =
        xmlDataIntent.containsKey("action") ? xmlDataIntent.get("action") : Intent.ACTION_MAIN;

    boolean useTargetPackage = xmlDataIntent.containsKey("targetPackage");
    String targetPackage = useTargetPackage ? xmlDataIntent.get("targetPackage") : mPackageName;
  ...
  public Drawable getIcon(int density) {
    try {
      ...
      return mContext.getResources().getDrawableForDensity(R.drawable.ic_default_shortcut, density);
    }
  }
