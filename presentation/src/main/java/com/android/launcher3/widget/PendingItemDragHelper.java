...
/**
 * Extension of {@link DragPreviewProvider} with logic specific to pending widgets/shortcuts dragged
...
public class PendingItemDragHelper extends DragPreviewProvider {
   ...
   *
   * @param previewBounds The bounds where the image was displayed, {@link
  ...
  public void startDrag(
    ...
    if (mAddInfo instanceof PendingAddWidgetInfo) {
      ...
      if (mPreview != null) {
        preview =
            LivePreviewWidgetCell.generateFromRemoteViews(
                launcher, mPreview, createWidgetInfo.info, maxWidth, previewSizeBeforeScale);
      }
      if (preview == null) {
        preview =
            app.getWidgetCache()
  ...
  public Bitmap createDragOutline(Canvas canvas) {
    if (mAddInfo instanceof PendingAddShortcutInfo) {
      ...
      Bitmap b =
          Bitmap.createBitmap(
              width + blurSizeOutline, height + blurSizeOutline, Bitmap.Config.ALPHA_8);
      canvas.setBitmap(b);
    ...
    float scaleFactor =
        Math.min(
            (w - blurSizeOutline) / (float) mPreviewBitmap.getWidth(),
