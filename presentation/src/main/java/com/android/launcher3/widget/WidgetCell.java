 ...
 *
 * <p>This view does not support padding. Since the image is scaled down to fit the view, padding
 * will further decrease the scaling factor. Drag-n-drop uses the view bounds for showing a smooth
 * transition from the view to drag view, so when adding padding support, DnD would need to consider
...
public class WidgetCell extends LinearLayout implements OnLayoutChangeListener {
  ...
  public void applyFromCellItem(WidgetItem item, WidgetPreviewLoader loader) {
    ...
    mWidgetDims.setText(
        getContext().getString(R.string.widget_dims_format, mItem.spanX, mItem.spanY));
    mWidgetDims.setContentDescription(
        getContext().getString(R.string.widget_accessible_dims_format, mItem.spanX, mItem.spanY));
  ...
  public void applyPreview(Bitmap bitmap, boolean animate) {
    if (bitmap != null) {
      mWidgetImage.setBitmap(
          bitmap, DrawableFactory.get(getContext()).getBadgeForUser(mItem.user, getContext()));
      if (mAnimatePreview) {
