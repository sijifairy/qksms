 ...
 * (becuase it causes an extra configuration changed) - From all apps - From the center workspace -
...
public class LauncherStateTransitionAnimation {
  ...
  private static class PrivateTransitionCallbacks {
    ...
    AnimatorListenerAdapter getMaterialRevealViewAnimatorListener(
        View revealView, View buttonView) {
      return null;
    }
   ...
   *
   * @param startSearchAfterTransition Immediately starts app search after the transition to All
   *     Apps is completed.
   */
  public void startAnimationToAllApps(
    ...
    PrivateTransitionCallbacks cb =
        new PrivateTransitionCallbacks(1f) {
          @Override
  ...
  public void startAnimationToWorkspace(
      final Launcher.State fromState,
    ...
    if (fromState == Launcher.State.APPS
        || fromState == Launcher.State.APPS_SPRING_LOADED
        || mAllAppsController.isTransitioning()) {
      startAnimationToWorkspaceFromAllApps(
          fromWorkspaceState, toWorkspaceState, animated, PULLUP, onCompleteRunnable);
      ...
      startAnimationToWorkspaceFromWidgets(
          fromWorkspaceState, toWorkspaceState, animated, onCompleteRunnable);
    } else {
  ...
  private void startAnimationToOverlay(
      ...
      final BaseContainerView toView,
      final boolean animated,
      int animType,
    ...
    playCommonTransitionAnimations(toWorkspaceState, animated, initialized, animation, layerViews);
    ...
    if (animType == CIRCULAR_REVEAL) {
      ...
      ObjectAnimator pageDrift =
          ObjectAnimator.ofFloat(contentView, "translationY", revealViewToYDrift, 0);
      ...
      Animator reveal =
          new CircleRevealOutlineProvider(width / 2, height / 2, startRadius, revealRadius)
              .createRevealAnimator(revealView);
  ...
  private void playCommonTransitionAnimations(
    ...
    Animator workspaceAnim =
        mLauncher.startWorkspaceStateChangeAnimation(toWorkspaceState, animated, layerViews);
  ...
  private void startAnimationToWorkspaceFromAllApps(
      final Workspace.State fromWorkspaceState,
      final Workspace.State toWorkspaceState,
      final boolean animated,
      int type,
  ...
  private void startAnimationToWorkspaceFromWidgets(
      final Workspace.State fromWorkspaceState,
      final Workspace.State toWorkspaceState,
      final boolean animated,
      final Runnable onCompleteRunnable) {
  ...
  private void startAnimationToWorkspaceFromOverlay(
      ...
      final Workspace.State toWorkspaceState,
      final View buttonView,
      final BaseContainerView fromView,
      final boolean animated,
      int animType,
      final Runnable onCompleteRunnable,
      final PrivateTransitionCallbacks pCb) {
    ...
    if (animType == CIRCULAR_REVEAL) {
      ...
      if (fromView.getVisibility() == View.VISIBLE) {
        ...
        ObjectAnimator panelDriftY =
            ObjectAnimator.ofFloat(revealView, "translationY", 0, revealViewToYDrift);
        panelDriftY.setDuration(revealDuration - SINGLE_FRAME_DELAY);
        panelDriftY.setStartDelay(itemsAlphaStagger + SINGLE_FRAME_DELAY);
        panelDriftY.setInterpolator(decelerateInterpolator);
        animation.play(panelDriftY);

        ObjectAnimator panelDriftX =
            ObjectAnimator.ofFloat(revealView, "translationX", 0, revealViewToXDrift);
        ...
        if (pCb.materialRevealViewFinalAlpha != 1f) {
          ObjectAnimator panelAlpha =
              ObjectAnimator.ofFloat(revealView, "alpha", 1f, pCb.materialRevealViewFinalAlpha);
        ...
        // Create the individual animators
        ObjectAnimator pageDrift =
            ObjectAnimator.ofFloat(contentView, "translationY", 0, revealViewToYDrift);
        ...
        invalidateScrim.addUpdateListener(
        ...
        AnimatorListenerAdapter listener =
            pCb.getMaterialRevealViewAnimatorListener(revealView, buttonView);
        Animator reveal =
            new CircleRevealOutlineProvider(width / 2, height / 2, revealRadius, finalRadius)
                .createRevealAnimator(revealView);
        reveal.setInterpolator(new LogDecelerateInterpolator(100, 0));
        reveal.setDuration(revealDuration);
