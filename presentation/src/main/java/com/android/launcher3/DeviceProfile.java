...
public class DeviceProfile {
  ...
  /**
   * The maximum amount of left/right workspace padding as a percentage of the screen width. To be
   * clear, this means that up to 7% of the screen width can be used as left padding, and 7% of the
   * screen width can be used as right padding.
   */
  ...
  public DeviceProfile(
    ...
    pageIndicatorLandLeftNavBarGutterPx =
        res.getDimensionPixelSize(
            R.dimen.dynamic_grid_page_indicator_land_left_nav_bar_gutter_width);
    pageIndicatorLandRightNavBarGutterPx =
        res.getDimensionPixelSize(
    ...
    hotseatBarLeftNavBarRightPaddingPx =
        res.getDimensionPixelSize(R.dimen.dynamic_grid_hotseat_land_left_nav_bar_right_padding);
    hotseatBarRightNavBarRightPaddingPx =
        res.getDimensionPixelSize(R.dimen.dynamic_grid_hotseat_land_right_nav_bar_right_padding);
    hotseatBarLeftNavBarLeftPaddingPx =
        res.getDimensionPixelSize(R.dimen.dynamic_grid_hotseat_land_left_nav_bar_left_padding);
    hotseatBarRightNavBarLeftPaddingPx =
        res.getDimensionPixelSize(R.dimen.dynamic_grid_hotseat_land_right_nav_bar_left_padding);
    hotseatBarSizePx =
        isVerticalBarLayout()
  ...
  DeviceProfile getMultiWindowProfile(Context context, Point mwSize) {
    ...
    DeviceProfile profile =
        new DeviceProfile(context, inv, mwSize, mwSize, mwSize.x, mwSize.y, isLandscape);
  ...
  /**
   * Adjusts the profile so that the labels on the Workspace are hidden. It is important to call
   * this method after the All Apps variables have been set.
   */
  ...
  private void updateIconSize(float scale, Resources res, DisplayMetrics dm) {
    ...
    cellHeightPx =
        iconSizePx + iconDrawablePaddingPx + Utilities.calculateTextHeight(iconTextSizePx);
    int cellYPadding = (getCellSize().y - cellHeightPx) / 2;
    if (iconDrawablePaddingPx > cellYPadding && !isVerticalBarLayout() && !inMultiWindowMode()) {
    ...
    if (!isVerticalBarLayout()) {
      int expectedWorkspaceHeight =
          availableHeightPx - hotseatBarSizePx - pageIndicatorSizePx - topWorkspacePadding;
  ...
  private void updateFolderCellSize(float scale, DisplayMetrics dm, Resources res) {
    ...
    folderChildDrawablePaddingPx =
        Math.max(0, (folderCellHeightPx - folderChildIconSizePx - textHeight) / 3);
  }
  ...
  public Point getSearchBarDimensForWidgetOpts() {
    if (isVerticalBarLayout()) {
      ...
      if (isTablet) {
        ...
        gap =
            ((width - 2 * edgeMarginPx - (inv.numColumns * cellWidthPx))
                    / (2 * (inv.numColumns + 1)))
                + edgeMarginPx;
      } else {
  ...
  public Point getCellSize() {
    ...
    result.x =
        calculateCellWidth(
            availableWidthPx - padding.x - cellLayoutPaddingLeftRightPx * 2, inv.numColumns);
    result.y =
        calculateCellHeight(availableHeightPx - padding.y - cellLayoutBottomPaddingPx, inv.numRows);
    return result;
  }
  ...
  public Rect getWorkspacePadding(Rect recycle) {
    ...
    if (isVerticalBarLayout()) {
      if (mInsets.left > 0) {
        padding.set(
            mInsets.left + pageIndicatorLandLeftNavBarGutterPx,
            0,
            hotseatBarSizePx
                + hotseatBarLeftNavBarRightPaddingPx
                + hotseatBarLeftNavBarLeftPaddingPx
      ...
      if (isTablet) {
        ...
        int availablePaddingX =
            Math.max(
                0, width - ((inv.numColumns * cellWidthPx) + ((inv.numColumns - 1) * cellWidthPx)));
        availablePaddingX =
            (int) Math.min(availablePaddingX, width * MAX_HORIZONTAL_PADDING_PERCENT);
        int availablePaddingY =
            Math.max(
                0,
                height
                    - topWorkspacePadding
        ...
        padding.set(
            availablePaddingX / 2,
            topWorkspacePadding + availablePaddingY / 2,
            availablePaddingX / 2,
  ...
  public Rect getAbsoluteOpenFolderBounds() {
    if (isVerticalBarLayout()) {
      ...
      return new Rect(
          mInsets.left + dropTargetBarSizePx + edgeMarginPx,
          mInsets.top,
      ...
      return new Rect(
          ...
          mInsets.top + availableHeightPx - hotseatBarSizePx - pageIndicatorSizePx - edgeMarginPx);
    }
  }
  ...
  int getOverviewModeButtonBarHeight() {
    ...
    return Utilities.boundToRange(
        zoneHeight, overviewModeMinIconZoneHeightPx, overviewModeMaxIconZoneHeightPx);
  }
  ...
  /**
   * When {@code true}, the device is in landscape mode and the hotseat is on the right column. When
   * {@code false}, either device is in portrait mode or the device is in landscape mode and the
   * hotseat is on the bottom row.
   */
  ...
  public void layout(Launcher launcher, boolean notifyListeners) {
    ...
    if (hasVerticalBarLayout) {
      ...
      int paddingRight =
          mInsets.left > 0
              ...
              : hotseatBarRightNavBarRightPaddingPx;
      int paddingLeft =
          mInsets.left > 0 ? hotseatBarLeftNavBarLeftPaddingPx : hotseatBarRightNavBarLeftPaddingPx;

      lp.gravity = Gravity.RIGHT;
      lp.width = hotseatBarSizePx + mInsets.left + mInsets.right + paddingLeft + paddingRight;
      lp.height = LayoutParams.MATCH_PARENT;

      hotseat
          .getLayout()
          .setPadding(
              mInsets.left + cellLayoutPaddingLeftRightPx + paddingLeft,
              mInsets.top,
    ...
    view.setPadding(
        paddingLeftRight, view.getPaddingTop(), paddingLeftRight, view.getPaddingBottom());
  ...
  private int getCurrentWidth() {
    return isLandscape ? Math.max(widthPx, heightPx) : Math.min(widthPx, heightPx);
  }

  private int getCurrentHeight() {
    return isLandscape ? Math.min(widthPx, heightPx) : Math.max(widthPx, heightPx);
  }
