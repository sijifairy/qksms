...
public class FocusHelper {
  ...
  public static class PagedFolderKeyEventListener implements View.OnKeyListener {
    ...
    public boolean onKey(View v, int keyCode, KeyEvent e) {
      ...
      if (DEBUG) {
        Log.v(
            TAG,
            String.format("Handle ALL Folders keyevent=[%s].", KeyEvent.keyCodeToString(keyCode)));
      }
      ...
      // Process focus.
      int newIconIndex =
          FocusLogic.handleKeyEvent(keyCode, matrix, iconIndex, pageIndex, pageCount, isLayoutRtl);
      ...
      switch (newIconIndex) {
        ...
        case FocusLogic.PREVIOUS_PAGE_LEFT_COLUMN:
          ...
          if (newParent != null) {
            ...
            child =
                newParent.getChildAt(
                    ((newIconIndex == FocusLogic.PREVIOUS_PAGE_LEFT_COLUMN)
                            ^ newParent.invertLayoutHorizontally())
                        ? 0
                        : matrix.length - 1,
                    row);
          }
   ...
   *
   * <p>Currently we don't special case for the phone UI in different orientations, even though the
   * hotseat is on the side in landscape mode. This is to ensure that accessibility consistency is
   * maintained across rotations.
   */
  static boolean handleHotseatButtonKeyEvent(View v, int keyCode, KeyEvent e) {
    ...
    int iconRank =
        ((CellLayout.LayoutParams)
                hotseatLayout.getShortcutsAndWidgets().getChildAt(iconIndex).getLayoutParams())
            .cellX;
    ...
    // Process the focus.
    int newIconIndex =
        FocusLogic.handleKeyEvent(
            keyCode, matrix, iconIndex, pageIndex, pageCount, Utilities.isRtl(v.getResources()));
    ...
    switch (newIconIndex) {
      ...
      case FocusLogic.PREVIOUS_PAGE_RIGHT_COLUMN:
        ...
        boolean isPrevPageFullscreen =
            ((CellLayout.LayoutParams)
                    prevPage.getShortcutsAndWidgets().getChildAt(0).getLayoutParams())
                .isFullscreen;
      ...
      case FocusLogic.NEXT_PAGE_RIGHT_COLUMN:
        ...
        boolean isNextPageFullscreen =
            ((CellLayout.LayoutParams)
                    nextPage.getShortcutsAndWidgets().getChildAt(0).getLayoutParams())
                .isFullscreen;
  ...
  static boolean handleIconKeyEvent(View v, int keyCode, KeyEvent e) {
    ...
    if (DEBUG) {
      Log.v(
          TAG,
          String.format(
              "Handle WORKSPACE ICONS keyevent=[%s] isVerticalBar=%s",
              KeyEvent.keyCodeToString(keyCode), profile.isVerticalBarLayout()));
    }
    ...
    switch (newIconIndex) {
      ...
      case FocusLogic.NEXT_PAGE_RIGHT_COLUMN:
        ...
        if (parent != null) {
          ...
          matrix =
              FocusLogic.createSparseMatrixWithPivotColumn(iconLayout, iconLayout.getCountX(), row);
          newIconIndex =
              FocusLogic.handleKeyEvent(
                  keyCode,
                  matrix,
  ...
  private static View handlePreviousPageLastItem(
      Workspace workspace, CellLayout hotseatLayout, int pageIndex, boolean isRtl) {
    if (pageIndex - 1 < 0) {
      return null;
    }
  ...
  private static View handleNextPageFirstItem(
      Workspace workspace, CellLayout hotseatLayout, int pageIndex, boolean isRtl) {
