...
public class LauncherProvider extends ContentProvider {
  ...
  private boolean initializeExternalAdd(ContentValues values) {
    ...
    if (itemType != null
      ...
      ComponentName cn =
          ComponentName.unflattenFromString(values.getAsString(Favorites.APPWIDGET_PROVIDER));
      ...
      case LauncherSettings.Settings.METHOD_SET_EXTRACTED_COLORS_AND_WALLPAPER_ID:
        {
          String extractedColors =
              extras.getString(LauncherSettings.Settings.EXTRA_EXTRACTED_COLORS);
  ...
  private ArrayList<Long> deleteEmptyFolders() {
    ...
    try (SQLiteTransaction t = new SQLiteTransaction(db)) {
      ...
      if (!folderIds.isEmpty()) {
        db.delete(
            Favorites.TABLE_NAME,
            Utilities.createDbSelectionQuery(LauncherSettings.Favorites._ID, folderIds),
            null);
      }
      t.commit();
  ...
  /**
   * Loads the default workspace based on the following priority scheme: 1) From the app
   * restrictions 2) From a package provided by play store 3) From a partner configuration APK,
   * already in the system image 4) The default configuration for the particular device
   */
    ...
    if (sp.getBoolean(EMPTY_DATABASE_CREATED, false)) {
      ...
      if (loader == null) {
        ...
        if (partner != null && partner.hasDefaultLayout()) {
          ...
          if (workspaceResId != 0) {
            loader =
                new DefaultLayoutParser(
                    getContext(), widgetHost, mOpenHelper, partnerRes, workspaceResId);
          }
        }
      }
      ...
      if ((mOpenHelper.loadFavorites(mOpenHelper.getWritableDatabase(), loader) <= 0)
        ...
        mOpenHelper.loadFavorites(
            mOpenHelper.getWritableDatabase(), getDefaultLayoutParser(widgetHost));
      }
      clearFlagEmptyDbCreated();
    }
  }
  ...
  private DefaultLayoutParser getDefaultLayoutParser(AppWidgetHost widgetHost) {
    ...
    return new DefaultLayoutParser(
        getContext(), widgetHost, mOpenHelper, getContext().getResources(), defaultLayout);
  }
  ...
  public static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
    ...
    protected void onEmptyDbCreated() {
      ...
      if (mWidgetHostResetHandler != null) {
        ...
        mWidgetHostResetHandler.sendEmptyMessage(ChangeListenerWrapper.MSG_APP_WIDGET_HOST_RESET);
      }
    ...
    public long getDefaultUserSerial() {
      return UserManagerCompat.getInstance(mContext).getSerialNumberForUser(Process.myUserHandle());
    }
    ...
    public void onOpen(SQLiteDatabase db) {
      ...
      DbDowngradeHelper.updateSchemaFile(
          schemaFile, SCHEMA_VERSION, mContext, R.raw.downgrade_schema);
    }

    /**
     * One-time data updated before support of onDowngrade was added. This update is backwards
     * compatible and can safely be run multiple times. Note: No new logic should be added here
     * after release, as the new logic might not get executed on an existing device. TODO: Move this
     * to db upgrade path, once the downgrade path is released.
     */
    protected void handleOneTimeDataUpgrade(SQLiteDatabase db) {
      ...
      for (UserHandle user : um.getUserProfiles()) {
        ...
        String sql =
            "update favorites set intent = replace(intent, "
                + "';l.profile="
                + serial
                + ";', ';') where itemType = 0;";
        db.execSQL(sql);
        ...
        case 14:
          {
            try (SQLiteTransaction t = new SQLiteTransaction(db)) {
              ...
              db.execSQL(
                  "ALTER TABLE favorites " + "ADD COLUMN modified INTEGER NOT NULL DEFAULT 0;");
              db.execSQL(
                  "ALTER TABLE workspaceScreens "
    ...
    public void onDowngrade(SQLiteDatabase db, int oldVersion, int newVersion) {
      try {
        ...
        Log.d(
            TAG,
            "Unable to downgrade from: " + oldVersion + " to " + newVersion + ". Wiping databse.",
            e);
        createEmptyDB(db);
      }
    }
    ...
    /**
     * Removes widgets which are registered to the Launcher's host, but are not present in our
     * model.
     */
    ...
    public boolean recreateWorkspaceTable(SQLiteDatabase db) {
      try (SQLiteTransaction t = new SQLiteTransaction(db)) {
        ...
        try (Cursor c =
          ...
          sortedIDs =
              new ArrayList<>(LauncherDbUtils.iterateCursor(c, 0, new LinkedHashSet<Long>()));
        }
        db.execSQL("DROP TABLE IF EXISTS " + WorkspaceScreens.TABLE_NAME);
        addWorkspacesTable(db, false);
    ...
    boolean updateFolderItemsRank(SQLiteDatabase db, boolean addRankColumn) {
      try (SQLiteTransaction t = new SQLiteTransaction(db)) {
        ...
        Cursor c =
            db.rawQuery(
                "SELECT container, MAX(cellX) FROM favorites"
    ...
    private boolean addIntegerColumn(SQLiteDatabase db, String columnName, long defaultValue) {
      try (SQLiteTransaction t = new SQLiteTransaction(db)) {
        db.execSQL(
            "ALTER TABLE favorites ADD COLUMN "
                + columnName
