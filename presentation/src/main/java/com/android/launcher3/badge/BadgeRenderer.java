...
public class BadgeRenderer {
  ...
  /**
   * Draw a circle in the top right corner of the given bounds, and draw {@link
   * BadgeInfo#getNotificationCount()} on top of the circle.
   *
  ...
  public void draw(
    ...
    IconDrawer iconDrawer =
        badgeInfo != null && badgeInfo.isIconLarge() ? mLargeIconDrawer : mSmallIconDrawer;
    Shader icon =
        badgeInfo == null
            ? null
            : badgeInfo.getNotificationIconForBadge(
                mContext, palette.backgroundColor, mSize, iconDrawer.mPadding);
    String notificationCount =
        badgeInfo == null ? "0" : String.valueOf(badgeInfo.getNotificationCount());
    ...
    if (backgroundWithShadow == null) {
      backgroundWithShadow =
          new ShadowGenerator.Builder(Color.WHITE).setupBlurForSize(mSize).createPill(width, mSize);
