...
public class AllAppsCaretController {
  ...
  public AllAppsCaretController(CaretDrawable caret, Launcher launcher) {
    ...
    final long caretAnimationDuration =
        launcher.getResources().getInteger(R.integer.config_caretAnimationDuration);
    final Interpolator caretInterpolator =
        AnimationUtils.loadInterpolator(launcher, android.R.interpolator.fast_out_slow_in);
  ...
  public void updateCaret(float containerProgress, float velocity, boolean dragging) {
    ...
    if (getThreshold() < containerProgress
        && containerProgress < 1 - getThreshold()
