...
/**
 * Handles AllApps view transition. 1) Slides all apps view using direct manipulation 2) When finger
 * is released, animate to either top or bottom accordingly.
 *
 * <p>Algorithm: If release velocity > THRES1, snap according to the direction of movement. If
 * release velocity < THRES1, snap according to either top or bottom depending on whether it's
...
public class AllAppsTransitionController
  ...
  public boolean onControllerInterceptTouchEvent(MotionEvent ev) {
    ...
    if (mDetector.isSettlingState()
        && (isInDisallowRecatchBottomZone() || isInDisallowRecatchTopZone())) {
      return false;
    }
    ...
    if (mNotificationState != NotificationState.Locked) {
      if (mProgress < 1f) {
        ...
        if (velocity > NOTIFICATION_OPEN_VELOCITY
            && (mNotificationState == NotificationState.Free
                || mNotificationState == NotificationState.Closed)) {
          mNotificationState =
              openNotifications() ? NotificationState.Opened : NotificationState.Locked;
        } else if (velocity < NOTIFICATION_CLOSE_VELOCITY
            && mNotificationState == NotificationState.Opened) {
          mNotificationState =
              closeNotifications() ? NotificationState.Closed : NotificationState.Locked;
        }

        // Don't open all apps when notification shade is being used
        if (mNotificationState == NotificationState.Opened
            || mNotificationState == NotificationState.Closed) {
          return true;
        }
      }
  ...
  public void onDragEnd(float velocity, boolean fling) {
    ...
    final int containerType =
        mTouchEventStartedOnHotseat ? ContainerType.HOTSEAT : ContainerType.WORKSPACE;
    ...
    if (fling
      ...
      if (velocity < 0) {
        ...
        if (!mLauncher.isAllAppsVisible()) {
          mLauncher
              ...
              .logActionOnContainer(Action.Touch.FLING, Action.Direction.UP, containerType);
        }
        mLauncher.showAppsView(
      ...
      if (mAppsView.getTranslationY() > mShiftRange / 2) {
        ...
        if (!mLauncher.isAllAppsVisible()) {
          mLauncher
              .getUserEventDispatcher()
              .logActionOnContainer(Action.Touch.SWIPE, Action.Direction.UP, containerType);
        }
        mLauncher.showAppsView(
            true, /* animated */ false /* updatePredictedApps */, false /* focusSearchBar */);
      }
  ...
  private void updateLightStatusBar(float shift) {
    ...
    if (forceChange) {
      mLauncher
          ...
          .updateUiState(SystemUiController.UI_STATE_ALL_APPS, !mIsDarkTheme);
    } else {
      mLauncher.getSystemUiController().updateUiState(SystemUiController.UI_STATE_ALL_APPS, 0);
    }
  }
  ...
  public void setProgress(float progress) {
    ...
    int color =
        (Integer)
            mEvaluator.evaluate(
                ...
                mHotseatBackgroundColor,
                mAllAppsBackgroundColor);
    int bgAlpha =
        Color.alpha(
            (int) mEvaluator.evaluate(alpha, mHotseatBackgroundColor, mAllAppsBackgroundColor));
    ...
    if (!mLauncher.getDeviceProfile().isVerticalBarLayout()) {
      mWorkspace.setHotseatTranslationAndAlpha(
          Workspace.Direction.Y, -mShiftRange + shiftCurrent, hotseatAlpha);
    } else {
      mWorkspace.setHotseatTranslationAndAlpha(
          Workspace.Direction.Y,
    ...
    if (!mDetector.isDraggingState()) {
      mContainerVelocity =
          mDetector.computeVelocity(shiftCurrent - shiftPrevious, System.currentTimeMillis());
    }
  ...
  public void showDiscoveryBounce() {
    ...
    mDiscoBounceAnimation = AnimatorInflater.loadAnimator(mLauncher, R.animator.discovery_bounce);
    mDiscoBounceAnimation.addListener(
        new AnimatorListenerAdapter() {
          @Override
  ...
  public void setupViews(AllAppsContainerView appsView, Hotseat hotseat, Workspace workspace) {
    ...
    mCaretController =
        new AllAppsCaretController(mWorkspace.getPageIndicator().getCaretDrawable(), mLauncher);
