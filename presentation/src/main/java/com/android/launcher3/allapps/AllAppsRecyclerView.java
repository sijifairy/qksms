...
public class AllAppsRecyclerView extends BaseRecyclerView implements LogContainerProvider {
  ...
  public AllAppsRecyclerView(
      Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    super(context, attrs, defStyleAttr);
    Resources res = getResources();
    addOnItemTouchListener(this);
    mEmptySearchBackgroundTopOffset =
        res.getDimensionPixelSize(R.dimen.all_apps_empty_search_bg_top_offset);
  ...
  /**
   * Ensures that we can present a stable scrollbar for views of varying types by pre-measuring all
  ...
  public void preMeasureViews(AllAppsGridAdapter adapter) {
    ...
    putSameHeightFor(
        adapter, widthMeasureSpec, heightMeasureSpec, AllAppsGridAdapter.VIEW_TYPE_SEARCH_MARKET);
    putSameHeightFor(
        adapter, widthMeasureSpec, heightMeasureSpec, AllAppsGridAdapter.VIEW_TYPE_EMPTY_SEARCH);
    ...
    if (mApps.shouldShowEmptySearch()) {
      if (mEmptySearchBackground == null) {
        mEmptySearchBackground =
            DrawableFactory.get(getContext()).getAllAppsBackground(getContext());
  ...
  public boolean onInterceptTouchEvent(MotionEvent e) {
    ...
    if (!result
        ...
        && mEmptySearchBackground != null
        && mEmptySearchBackground.getAlpha() > 0) {
      ...
      if (!mScrollbar.isDraggingThumb()) {
        ...
        if (diffScrollY * dy > 0f) {
          ...
          if (dy < 0) {
            ...
            int offset =
                (int)
                    ((dy * (availableScrollBarHeight - thumbScrollY))
  ...
  public int getCurrentScrollY() {
    ...
    return getPaddingTop() + getCurrentScrollY(position, getLayoutManager().getDecoratedTop(child));
  }
    ...
    if (y < 0) {
      ...
      for (int i = 0; i < position; i++) {
        ...
        if (AllAppsGridAdapter.isIconViewType(item.viewType)) {
          ...
          if (posItem != null
              && posItem.viewType == item.viewType
  ...
  /**
   * Returns the available scroll height: AvailableScrollHeight = Total height of the all items -
   * last page height
   */
  @Override
  protected int getAvailableScrollHeight() {
    return getPaddingTop()
        + getCurrentScrollY(mApps.getAdapterItems().size(), 0)
        - getHeight()
        + getPaddingBottom();
  }
  ...
  private void updateEmptySearchBackgroundBounds() {
    ...
    mEmptySearchBackground.setBounds(
        x,
        y,
  ...
  private class OverScrollHelper implements SwipeDetector.Listener {
    ...
    private void reset(boolean shouldSpring) {
      ...
      if (Float.compare(y, 0) != 0) {
        ...
        ObjectAnimator.ofFloat(AllAppsRecyclerView.this, AllAppsRecyclerView.CONTENT_TRANS_Y, 0)
            .setDuration(100)
            .start();
      }
