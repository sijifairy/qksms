...
public class PendingAppWidgetHostView extends LauncherAppWidgetHostView
  ...
  public PendingAppWidgetHostView(
      Context context, LauncherAppWidgetInfo info, IconCache cache, boolean disabledForSafeMode) {
  ...
  /**
   * A pending widget is ready for setup after the provider is installed and 1) Widget id is not
   * valid: the widget id is not yet bound to the provider, probably because the launcher doesn't
   * have appropriate permissions. Note that we would still have an allocated id as that does not
   * require any permissions and can be done during view inflation. 2) UI is not ready: the id is
   * valid and the bound. But the widget has a configure activity which needs to be called once.
   */
  ...
  private void updateDrawableBounds() {
    ...
    if (mSettingIconDrawable == null) {
      ...
      if (availableWidth > 0) {
        ...
        mSetupTextLayout =
            new StaticLayout(
                getResources().getText(R.string.gadget_setup_text),
                mPaint,
                availableWidth,
        ...
        float minHeightWithText =
            textHeight + actualIconSize * settingIconScaleFactor + grid.iconDrawablePaddingPx;
