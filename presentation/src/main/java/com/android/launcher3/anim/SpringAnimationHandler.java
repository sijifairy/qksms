 ...
 * {@link MotionEvent}s.
 *
 * <p>Supports setting either X or Y velocity on the list of springs added to this handler.
 */
public class SpringAnimationHandler<T> {
  ...
  /**
   * @param direction Either {@link #X_DIRECTION} or {@link #Y_DIRECTION}. Determines which
   ...
   * @param setDefaultValues If True, sets the spring to the default {@link AnimationFactory}
   *     values.
   */
   ...
   * @param startValue < 0 if scrolling from start to end; > 0 if scrolling from end to start The
   *     magnitude of the number changes how the spring will move.
   * @param setVelocity If true, we set the velocity to {@link #mCurrentVelocity} before starting
   *     the animation.
   */
  ...
  public void animateToPositionWithVelocity(float position, int startValue, float velocity) {
    if (DEBUG) {
      Log.d(
          TAG,
          "animateToPosition#pos=" + position + ", start=" + startValue + ", velocity=" + velocity);
    }
  ...
  /**
   * This interface is used to initialize and update the SpringAnimations added to the {@link
