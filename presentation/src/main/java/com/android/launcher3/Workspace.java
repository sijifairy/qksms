...
public class Workspace extends PagedView
  ...
  /**
   * Hotseat alpha can be changed when moving horizontally, vertically, changing states. The values
   * correspond to {@link Direction#X}, {@link Direction#Y} & {@link #HOTSEAT_STATE_ALPHA_INDEX}
   * respectively.
   */
  ...
  public void bindAndInitFirstWorkspaceScreen(View qsb) {
    ...
    if (FeatureFlags.PULLDOWN_SEARCH) {
      firstPage.setOnTouchListener(
          new VerticalFlingDetector(mLauncher) {
      ...
      firstPage.setOnInterceptTouchListener(
          new VerticalFlingDetector(mLauncher) {
  ...
  public void addToCustomContentPage(
    ...
    customContent.setOnFocusChangeListener(
        mLauncher.mFocusHandler.getHideIndicatorOnFocusListener());
  ...
  public long commitExtraEmptyScreen() {
    ...
    long newId =
        LauncherSettings.Settings.call(
                getContext().getContentResolver(), LauncherSettings.Settings.METHOD_NEW_SCREEN_ID)
  ...
  /**
   * At bind time, we use the rank (screenId) to compute x and y for hotseat items. See {@link
   * #addInScreen}.
   */
  ...
  /**
   * Adds the specified child in the specified screen based on the {@param info} See {@link
   * #addInScreen(View, long, long, int, int, int, int)}.
   */
  public void addInScreen(View child, ItemInfo info) {
    addInScreen(
        child, info.container, info.screenId, info.cellX, info.cellY, info.spanX, info.spanY);
  }
  ...
  protected void overScroll(float amount) {
    ...
    boolean shouldScrollOverlay =
        mLauncherOverlay != null && ((amount <= 0 && !mIsRtl) || (amount >= 0 && mIsRtl));
    ...
    if (Float.compare(scroll, 1f) == 0) {
      if (!mOverlayShown) {
        mLauncher
            ...
            .logActionOnContainer(
                Action.Touch.SWIPE, Action.Direction.LEFT, ContainerType.WORKSPACE, 0);
      }
      mOverlayShown = true;
      ...
      if (mOverlayShown) {
        mLauncher
            .getUserEventDispatcher()
            .logActionOnContainer(
                Action.Touch.SWIPE, Action.Direction.RIGHT, ContainerType.WORKSPACE, -1);
      }
      mOverlayShown = false;
    }
    float offset = 0f;
   ...
   * @param alpha the alpha for the workspace page
   */
  private void setWorkspaceTranslationAndAlpha(
      Direction direction, float translation, float alpha) {
  ...
  public ValueAnimator createHotseatAlphaAnimator(float finalValue) {
    if (Float.compare(finalValue, mHotseatAlpha[HOTSEAT_STATE_ALPHA_INDEX]) == 0) {
      ...
      ValueAnimator animator =
          ValueAnimator.ofFloat(mHotseatAlpha[HOTSEAT_STATE_ALPHA_INDEX], finalValue);
      ...
      AccessibilityManager am =
          (AccessibilityManager) mLauncher.getSystemService(Context.ACCESSIBILITY_SERVICE);
      final boolean accessibilityEnabled = am.isEnabled();
      animator.addUpdateListener(
          new AlphaUpdateListener(mLauncher.getHotseat(), accessibilityEnabled));
      animator.addUpdateListener(new AlphaUpdateListener(mPageIndicator, accessibilityEnabled));
      return animator;
    }
  }
  ...
  protected void notifyPageSwitchListener(int prevPage) {
    ...
    if (prevPage != mCurrentPage) {
      int swipeDirection =
          (prevPage < mCurrentPage) ? Action.Direction.RIGHT : Action.Direction.LEFT;
      mLauncher
          .getUserEventDispatcher()
          .logActionOnContainer(
              Action.Touch.SWIPE, swipeDirection, ContainerType.WORKSPACE, prevPage);
    }
  ...
  private void updateStateForCustomContent() {
    ...
    if (hasCustomContent()) {
      ...
      int scrollDelta =
          getScrollX() - getScrollForPage(index) - getLayoutTransitionOffsetForPage(index);
  ...
  void updateChildrenLayersEnabled(boolean force) {
    ...
    boolean enableChildrenLayers =
        force || small || mAnimatingViewIntoPlace || isPageInTransition();
  ...
  private void enableHwLayersOnVisiblePages() {
    if (mChildrenLayersEnabled) {
      ...
      if (mForceDrawAdjacentPages) {
        ...
        leftScreen = Utilities.boundToRange(getCurrentPage() - 1, numCustomPages(), rightScreen);
  ...
  /**
   * Sets the current workspace {@link State}, returning an animation transitioning the workspace to
   * that new state.
   */
  public Animator setStateWithAnimation(
      State toState, boolean animated, AnimationLayerSet layerViews) {
    ...
    AnimatorSet workspaceAnim =
        mStateTransitionAnimation.getAnimationToState(fromState, toState, animated, layerViews);

    boolean shouldNotifyWidgetChange = !fromState.shouldUpdateWidget && toState.shouldUpdateWidget;
  ...
  public void updateAccessibilityFlags() {
    ...
    if (!mLauncher.getAccessibilityDelegate().isInAccessibleDrag()) {
      ...
      setImportantForAccessibility(
          (mState == State.NORMAL || mState == State.OVERVIEW)
              ? IMPORTANT_FOR_ACCESSIBILITY_AUTO
              : IMPORTANT_FOR_ACCESSIBILITY_NO_HIDE_DESCENDANTS);
    }
  }
  ...
  public void startDrag(CellLayout.CellInfo cellInfo, DragOptions options) {
    ...
    if (options.isAccessibleDrag) {
      mDragController.addDragListener(
          new AccessibleDragListenerAdapter(this, CellLayout.WORKSPACE_ACCESSIBILITY_DRAG) {
            @Override
  ...
  public DragView beginDragShared(
      View child,
      DragSource source,
      ItemInfo dragObject,
    ...
    if (child instanceof BubbleTextView && !dragOptions.isAccessibleDrag) {
      PopupContainerWithArrow popupContainer =
          PopupContainerWithArrow.showForIcon((BubbleTextView) child);
  ...
  boolean willCreateUserFolder(
      ItemInfo info, CellLayout target, int[] targetCell, float distance, boolean considerTimeout) {
  ...
  boolean createUserFolderIfNecessary(
    ...
    if (aboveShortcut && willBecomeShortcut) {
      ...
      if (animate) {
        ...
        fi.performCreateAnimation(
            destInfo, v, sourceInfo, dragView, folderLocation, scale, postAnimationRunnable);
      } else {
  ...
  public void onDrop(final DragObject d) {
    ...
    if (d.dragSource != this) {
      ...
      if (dropTargetLayout != null && !d.cancelled) {
        ...
        long screenId =
            (mTargetCell[0] < 0) ? mDragInfo.screenId : getIdForScreen(dropTargetLayout);
        ...
        if (createUserFolderIfNecessary(
            cell, container, dropTargetLayout, mTargetCell, distance, false, d.dragView, null)) {
          return;
        }
        ...
        boolean returnToOriginalCellToPreventShuffling =
        ...
        if (foundCell
          ...
          AppWidgetResizeFrame.updateWidgetSizeRanges(
              awhv, mLauncher, resultSpan[0], resultSpan[1]);
        }
        ...
        if (foundCell) {
          ...
          if (hasMovedLayouts) {
            ...
            addInScreen(
                cell, container, screenId, mTargetCell[0], mTargetCell[1], info.spanX, info.spanY);
          }
          ...
          if (container != LauncherSettings.Favorites.CONTAINER_HOTSEAT
            ...
            LauncherAppWidgetProviderInfo pInfo =
                (LauncherAppWidgetProviderInfo) hostView.getAppWidgetInfo();
            if (pInfo != null
          ...
          mLauncher
              ...
              .modifyItemInDatabase(
                  info, container, screenId, lp.cellX, lp.cellY, item.spanX, item.spanY);
        } else {
          if (!returnToOriginalCellToPreventShuffling) {
            onNoCellFound(dropTargetLayout);
          }
      ...
      if (d.dragView.hasDrawn()) {
        ...
        if (isWidget) {
          ...
          animateWidgetDrop(
              info, parent, d.dragView, onCompleteRunnable, animationType, cell, false);
        } else {
          int duration = snapScreen < 0 ? -1 : ADJACENT_SCREEN_DROP_DURATION;
          mLauncher
              .getDragLayer()
              .animateViewIntoPosition(d.dragView, cell, duration, onCompleteRunnable, this);
        }
      } else {
  ...
  private void manageFolderFeedback(
    ...
    if (mDragMode == DRAG_MODE_NONE && userFolderPending && !mFolderCreationAlarm.alarmPending()) {
      ...
      FolderCreationAlarmListener listener =
          new FolderCreationAlarmListener(targetLayout, targetCell[0], targetCell[1]);
      ...
      if (dragObject.stateAnnouncer != null) {
        dragObject.stateAnnouncer.announce(
            WorkspaceAccessibilityHelper.getDescriptionForDropOver(dragOverView, getContext()));
      }
      return;
    }
   ...
   *
   * <p>NOTE: This can also be called when we are outside of a drag event, when we want to add an
   * item to one of the workspace screens.
   */
  private void onDropExternal(final int[] touchXY, final CellLayout cellLayout, DragObject d) {
    final Runnable exitSpringLoadedRunnable =
        new Runnable() {
          @Override
          public void run() {
    ...
    if (d.dragInfo instanceof PendingAddShortcutInfo) {
      ShortcutInfo si = ((PendingAddShortcutInfo) d.dragInfo).activityInfo.createShortcutInfo();
      if (si != null) {
        d.dragInfo = si;
      }
    }
    ...
    if (info instanceof PendingAddItemInfo) {
      ...
      if (pendingInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT) {
        mTargetCell =
            findNearestArea(touchXY[0], touchXY[1], spanX, spanY, cellLayout, mTargetCell);
        float distance =
            cellLayout.getDistanceFromCell(
                mDragViewVisualCenter[0], mDragViewVisualCenter[1], mTargetCell);
        if (willCreateUserFolder(d.dragInfo, cellLayout, mTargetCell, distance, true)
            || willAddToExistingUserFolder(d.dragInfo, cellLayout, mTargetCell, distance)) {
      ...
      Runnable onAnimationCompleteRunnable =
          new Runnable() {
            ...
            public void run() {
              ...
              mLauncher.addPendingItem(
                  pendingInfo, container, screenId, mTargetCell, item.spanX, item.spanY);
            }
          };
      boolean isWidget =
      ...
      AppWidgetHostView finalView =
          isWidget ? ((PendingAddWidgetInfo) pendingInfo).boundWidget : null;
      ...
      if (isWidget
          && ((PendingAddWidgetInfo) pendingInfo).info != null
      ...
      mLauncher
          .getModelWriter()
          .addOrMoveItemInDatabase(info, container, screenId, mTargetCell[0], mTargetCell[1]);

      addInScreen(
          view, container, screenId, mTargetCell[0], mTargetCell[1], info.spanX, info.spanY);
      ...
      if (d.dragView != null) {
        ...
        mLauncher
            .getDragLayer()
            .animateViewIntoPosition(d.dragView, view, exitSpringLoadedRunnable, this);
  ...
  private void getFinalPositionForDropAnimation(
    ...
    if (scale) {
      ...
      // the final location.
      loc[0] -=
          (dragView.getMeasuredWidth() - cellLayoutScale * r.width()) / 2
  ...
  public void animateWidgetDrop(
    ...
    getFinalPositionForDropAnimation(
        finalPos, scaleXY, dragView, cellLayout, info, mTargetCell, scalePreview);
  ...
  @Thunk
  int[] findNearestArea(
      int pixelX, int pixelY, int spanX, int spanY, CellLayout layout, int[] recycle) {
  ...
  public void onDropCompleted(
    ...
    if (success && !(beingCalledAfterUninstall && !mUninstallSuccessful)) {
      ...
      final CellLayout cellLayout =
          mLauncher.getCellLayout(mDragInfo.container, mDragInfo.screenId);
      if (cellLayout != null) {
    ...
    if (!isFlingToDelete) {
      ...
      mLauncher.exitSpringLoadedDragModeDelayed(
          success, Launcher.EXIT_SPRINGLOADED_MODE_SHORT_TIMEOUT, mDelayedResizeRunnable);
      mDelayedResizeRunnable = null;
    }
  }
          ...
          public boolean evaluate(ItemInfo info, View v) {
            if (info instanceof FolderInfo
              ...
              for (ShortcutInfo si : ((FolderInfo) info).contents) {
                folderBadgeInfo.addBadgeInfo(
                    mLauncher.getPopupDataProvider().getBadgeInfoForItem(si));
              }
  ...
  public void widgetsRestored(final ArrayList<LauncherAppWidgetInfo> changedInfo) {
    if (!changedInfo.isEmpty()) {
      DeferredWidgetRefresh widgetRefresh =
          new DeferredWidgetRefresh(changedInfo, mLauncher.getAppWidgetHost());

      LauncherAppWidgetInfo item = changedInfo.get(0);
      final AppWidgetProviderInfo widgetInfo;
      if (item.hasRestoreFlag(LauncherAppWidgetInfo.FLAG_ID_NOT_VALID)) {
        widgetInfo =
            AppWidgetManagerCompat.getInstance(mLauncher)
                .findProvider(item.providerName, item.user);
      } else {
        widgetInfo =
            AppWidgetManagerCompat.getInstance(mLauncher).getAppWidgetInfo(item.appWidgetId);
      }
  ...
  public void moveToScreenNextToCustomContent(boolean animate) {
    if (hasCustomContent()) {
      int ccIndex = getPageIndexForScreenId(CUSTOM_CONTENT_SCREEN_ID);
      if (getNextPage() != ccIndex) {
        return;
      }
      int nextToIndex = ++ccIndex;
      if (animate) {
        snapToPage(nextToIndex);
      } else {
        setCurrentPage(nextToIndex);
      }
      View child = getChildAt(nextToIndex);
      if (child != null) {
        child.requestFocus();
      }
    }
  ...
  private String getPageDescription(int page) {
    ...
    return getContext().getString(R.string.workspace_scroll_format, page + 1 - delta, nScreens);
  }
