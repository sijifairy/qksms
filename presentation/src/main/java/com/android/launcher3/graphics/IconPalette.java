...
public class IconPalette {
  ...
  /**
   * Returns an IconPalette based on the badge_color in colors.xml. If that color is
   * Color.TRANSPARENT, then returns null instead.
   */
  ...
  /**
   * Resolves a color such that it has enough contrast to be used as the color of an icon or text on
   * the given background color.
   *
   * @return a color of the same hue with enough contrast against the background.
   *     <p>This was copied from com.android.internal.util.NotificationColorUtil.
   */
  ...
  private static String contrastChange(int colorOld, int colorNew, int bg) {
    return String.format(
        "from %.2f:1 to %.2f:1",
        ColorUtils.calculateContrast(colorOld, bg), ColorUtils.calculateContrast(colorNew, bg));
  }
