...
public class LauncherIcons {
    ...
    try {
      ...
      if (resources != null) {
        ...
        return createIconBitmap(
            resources.getDrawableForDensity(id, LauncherAppState.getIDP(context).fillResIconDpi),
            context);
      }
    } catch (Exception e) {
      // Icon not found.
  ...
  /**
   * Returns a bitmap suitable for the all apps view. The icon is badged for {@param user}. The
   * bitmap is also visually normalized with other icons.
   */
  ...
  public static Bitmap badgeIconForUser(Bitmap icon, UserHandle user, Context context) {
    if (user != null && !Process.myUserHandle().equals(user)) {
      ...
      Drawable badged = context.getPackageManager().getUserBadgedIcon(drawable, user);
  ...
  public static Bitmap createScaledBitmapWithoutShadow(
    ...
    if (!FeatureFlags.LAUNCHER3_DISABLE_ICON_NORMALIZATION) {
      ...
      if (Utilities.ATLEAST_OREO && iconAppTargetSdk >= Build.VERSION_CODES.O) {
        ...
        if (Utilities.ATLEAST_OREO && FeatureFlags.LEGACY_ICON_TREATMENT && !outShape[0]) {
          Drawable wrappedIcon = wrapToAdaptiveIconDrawable(context, icon, scale);
          if (wrappedIcon != icon) {
            icon = wrappedIcon;
  ...
  public static Bitmap createIconBitmap(Drawable icon, Context context, float scale) {
    synchronized (sCanvas) {
      ...
      if (Utilities.ATLEAST_OREO && icon instanceof AdaptiveIconDrawable) {
        int offset =
            Math.max((int) (ShadowGenerator.BLUR_FACTOR * iconBitmapSize), Math.min(left, top));
  ...
  /**
   * If the platform is running O but the app is not providing AdaptiveIconDrawable, then shrink the
   * legacy icon and set it as foreground. Use color drawable as background to create
   * AdaptiveIconDrawable.
   */
  static Drawable wrapToAdaptiveIconDrawable(Context context, Drawable drawable, float scale) {
    ...
    try {
      if (!(drawable instanceof AdaptiveIconDrawable)) {
        AdaptiveIconDrawable iconWrapper =
            (AdaptiveIconDrawable)
                context.getDrawable(R.drawable.adaptive_icon_drawable_wrapper).mutate();
  ...
  public static Bitmap createShortcutIcon(
      ShortcutInfoCompat shortcutInfo, Context context, boolean badged) {
    return createShortcutIcon(shortcutInfo, context, badged, null);
  }

  public static Bitmap createShortcutIcon(
      ShortcutInfoCompat shortcutInfo, Context context, final Bitmap fallbackIcon) {
  ...
  public static Bitmap createShortcutIcon(
    ...
    if (unbadgedDrawable != null) {
      unbadgedBitmap = LauncherIcons.createScaledBitmapWithoutShadow(unbadgedDrawable, context, 0);
    } else {
  ...
  public static Bitmap getShortcutInfoBadge(ShortcutInfoCompat shortcutInfo, IconCache cache) {
    ...
    if (cn != null) {
      ...
      appInfo.intent =
          new Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_LAUNCHER).setComponent(cn);
   ...
   * This allows the badging to be done based on the action bitmap size rather than the scaled
   * bitmap size.
   */
