...
public abstract class ShortcutConfigActivityInfo {
  ...
  /**
   * Return a shortcut info, if it can be created directly on drop, without requiring any {@link
  ...
  /**
   * Returns true if various properties ({@link #getLabel()}, {@link #getFullResIcon}) can be safely
   * persisted.
   */
  ...
  public static class ShortcutConfigActivityInfoVO extends ShortcutConfigActivityInfo {
    ...
    public boolean startConfigActivity(Activity activity, int requestCode) {
      ...
      IntentSender is =
          activity.getSystemService(LauncherApps.class).getShortcutConfigActivityIntent(mInfo);
      try {
