...
public class AppWidgetResizeFrame extends FrameLayout
  ...
  public AppWidgetResizeFrame(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    mBackgroundPadding =
        getResources().getDimensionPixelSize(R.dimen.resize_frame_background_padding);
  ...
  public void setupForWidget(
    ...
    LauncherAppWidgetProviderInfo info =
        (LauncherAppWidgetProviderInfo) widgetView.getAppWidgetInfo();
    ...
    if (!info.isCustomWidget) {
      mWidgetPadding =
          AppWidgetHostView.getDefaultPaddingForWidget(
              getContext(), widgetView.getAppWidgetInfo().provider, null);
    } else {
  ...
  public boolean beginResizeIfPointInRegion(int x, int y) {
    ...
    mBottomBorderActive =
        (y > getHeight() - mTouchTargetWidth + mBottomTouchRegionAdjustment) && verticalActive;

    boolean anyBordersActive =
        mLeftBorderActive || mRightBorderActive || mTopBorderActive || mBottomBorderActive;
  ...
  private void resizeWidgetIfNeeded(boolean onDismiss) {
    ...
    int hSpanDelta =
        mTempRange1.applyDeltaAndBound(
            mLeftBorderActive,
            mRightBorderActive,
            hSpanInc,
            mMinHSpan,
            mCellLayout.getCountX(),
            mTempRange2);
  ...
  private static class IntRange {
    ...
    /**
     * Moves either the start or end edge (but never both) by {@param delta} and sets the result in
     * {@param out}
     */
    ...
    /**
     * Applies delta similar to {@link #applyDelta(boolean, boolean, int, IntRange)}, with extra
     * conditions.
     *
     * @param minSize minimum size after with the moving edge should not be shifted any further. For
     *     eg, if delta = -3 when moving the endEdge brings the size to less than minSize, only
     *     delta = -2 will applied
     ...
     */
    public int applyDeltaAndBound(
        boolean moveStart, boolean moveEnd, int delta, int minSize, int maxEnd, IntRange out) {
